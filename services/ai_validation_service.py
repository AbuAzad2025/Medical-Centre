...
class AIValidationService:
            ...
            if '/' in bp:
                try:
                    ...
                    if systolic <= diastolic:
                        errors.append(
                            '⚠️ ضغط الدم غير منطقي (الانقباضي يجب أن يكون أكبر من الانبساطي)'
                        )
                except (ValueError, TypeError):
    ...
    def validate_financial_data(data: dict[str, Any]) -> tuple[bool, list[str], list[str]]:
        ...
        if 'paid_amount' in data and 'total_amount' in data:
            try:
                ...
                if paid > total * 1.5:
                    warnings.append(
                        f'⚠️ المبلغ المدفوع ({paid}) أكبر بكثير من المبلغ الإجمالي ({total})'
                    )
            except (ValueError, TypeError):
                pass
    ...
    @staticmethod
    def validate_system_logic(
        operation: str, data: dict[str, Any], db_session
        ...
        try:
            ...
            if operation == 'create_appointment':
                ...
                if doctor_id and appointment_time:
                    ...
                    doctor = (
                        db.session.execute(select(User).filter_by(id=doctor_id, role='doctor'))
                        .scalars()
                        .first()
                    )
                    if not doctor:
                    ...
                    if conflicting:
                        errors.append('⚠️ يوجد موعد آخر لنفس الطبيب في نفس الوقت')
            ...
            if operation == 'delete_patient':
                ...
                visits_count = db.session.execute(
                    select(func.count()).select_from(Visit).filter_by(patient_id=patient_id)
                ).scalar()

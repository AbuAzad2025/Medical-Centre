...
class SafetyAlert:
    ...
    override_requires: str | None = None  # e.g., 'super_admin', 'head_physician'
...
class ClinicalSafetyService:
    ...
    def check_prescription_safety(
        ...
        try:
            alerts.extend(
                ClinicalSafetyService._check_drug_interactions(
                    patient_id, proposed_items, tenant_id
                )
            )
        except Exception:
        ...
        try:
            alerts.extend(
                ClinicalSafetyService._check_contraindications(
                    patient_id, proposed_items, tenant_id
                )
            )
        except Exception:
        ...
        try:
            alerts.extend(
                ClinicalSafetyService._check_duplicate_therapy(
                    patient_id, proposed_items, tenant_id
                )
            )
        except Exception:
    ...
    def _check_allergies(
        patient_id: int, proposed_items: list[dict], tenant_id: int
    ) -> list[SafetyAlert]:
        ...
        allergies = (
            db.session.execute(
                select(PatientAllergy).filter_by(patient_id=patient_id, tenant_id=tenant_id)
            )
            .scalars()
            .all()
        )
        ...
        for item in proposed_items:
            ...
            try:
                ...
                ingredients = (
                    db.session.execute(select(DrugIngredient).filter_by(medication_id=drug_id))
                    .scalars()
                    .all()
                )
    ...
    def _check_drug_interactions(
        ...
        active_prescriptions = (
            db.session.execute(
                select(Prescription).filter_by(patient_id=patient_id, tenant_id=tenant_id)
            )
            .scalars()
            .all()
        )
        ...
        proposed_drug_ids = {
            item.get('drug_id') or item.get('medication_id') for item in proposed_items
        }
        ...
        for drug_a in proposed_drug_ids:
            for drug_b in current_drug_ids:
                ...
                interactions = (
                    db.session.execute(
                        select(DrugInteraction).filter(
                            db.or_(
                                db.and_(
                                    DrugInteraction.medication_a_id == drug_a,
                                    DrugInteraction.medication_b_id == drug_b,
                                ),
                                db.and_(
                                    DrugInteraction.medication_a_id == drug_b,
                                    DrugInteraction.medication_b_id == drug_a,
                                ),
                            )
                        )
                    )
    ...
    def _check_contraindications(
        ...
        problems = (
            db.session.execute(
                select(PatientProblem).filter_by(
                    patient_id=patient_id, tenant_id=tenant_id, status='ACTIVE'
                )
            )
            .scalars()
            .all()
        ...
        for item in proposed_items:
            ...
            contraindications = (
                db.session.execute(
                    select(MedicationContraindication).filter_by(medication_id=drug_id)
                )
                .scalars()
                .all()
            )
    ...
    def _check_pregnancy_safety(
        ...
        patient = (
            db.session.execute(select(Patient).filter_by(id=patient_id, tenant_id=tenant_id))
            .scalars()
            .first()
        )
        ...
        for item in proposed_items:
            ...
            if not drug_id:
                continue
            medication = (
                db.session.execute(select(Medication).filter_by(id=drug_id)).scalars().first()
            )
            if not medication:
                continue

...
class FHIRService:
    ...
    def get_all_patients(limit: int = 100) -> list:
        ...
        return (
            db.session.execute(select(Patient).filter_by(status='ACTIVE').limit(limit))
            .scalars()
            .all()
        )
    ...
    def get_all_encounters(limit: int = 100) -> list:
        ...
        return (
            db.session.execute(select(Visit).order_by(Visit.created_at.desc()).limit(limit))
            .scalars()
            .all()
        )
    ...
    def serialize_lab_observation(result: Any) -> dict:
        return {
            ...
            'id': str(result.id),
            'status': 'final'
            if getattr(result, 'result_status', '') == 'COMPLETED'
            else 'preliminary',
            'code': {'text': result.test_name or getattr(result, 'test_type', 'Lab Test')},
            'subject': {'reference': f'Patient/{result.patient_id}'},
            'encounter': {'reference': f'Encounter/{result.visit_id}'}
            if getattr(result, 'visit_id', None)
            else None,
            'valueQuantity': {
                'value': result.result_value,
                'unit': result.unit or '',
            }
            if getattr(result, 'result_value', None)
            else None,
    ...
    def serialize_radiology_observation(result: Any) -> dict:
        return {
            ...
            'code': {
                'text': result.test_name or getattr(result, 'procedure_name', 'Radiology Test')
            },
            'subject': {'reference': f'Patient/{result.patient_id}'},

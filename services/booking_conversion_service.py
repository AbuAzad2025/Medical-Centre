...
class AppointmentCheckinService:
    ...
    @staticmethod
    def create_walkin(
        patient_id: int, doctor_id: int | None = None, department_id: int | None = None
    ) -> dict:
        from models.visit import Visit
...
class OnlineBookingConversionService:
    ...
    def convert_to_visit(booking) -> dict:
        ...
        visit = Visit(
            ...
            status=VisitState.OPEN,
            visit_type='REGULAR',
            visit_date=getattr(booking, 'appointment_date', None) or datetime.now(UTC).date(),
    ...
    def convert_to_appointment(booking) -> dict:
        ...
        appt_date = getattr(booking, 'appointment_date', None) or datetime.now(UTC).date()
        appt_time = getattr(booking, 'appointment_time', None)
        if appt_time:
            starts_at = datetime.combine(appt_date, appt_time, tzinfo=UTC)
        else:
            starts_at = datetime.combine(appt_date, datetime.min.time(), tzinfo=UTC)
        ...
        booking.status = BookingState.CONVERTED
        safe_commit(
            db.session, error_message='Failed to convert booking to appointment', reraise=True
        )
    ...
    def convert_based_on_profile(booking, profile_code: str | None = None) -> dict:
        ...
        if profile_code in ('standalone_radiology',):
            return OnlineBookingConversionService._convert_to_radiology_order(booking)
        if profile_code in ('private_doctor_clinic',):
    ...
    def _convert_to_lab_order(booking):
        ...
        booking.status = BookingState.CONVERTED
        safe_commit(
            db.session, error_message='Failed to convert booking to lab order', reraise=True
        )
    ...
    def _convert_to_radiology_order(booking):
        ...
        booking.status = BookingState.CONVERTED
        safe_commit(
            db.session, error_message='Failed to convert booking to radiology order', reraise=True
        )

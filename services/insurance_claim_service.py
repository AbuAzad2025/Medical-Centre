...
class InsuranceClaimService:
    ...
    def list_claims(tenant_id: int | None) -> list[dict]:
        ...
        rows = db.session.execute(q.order_by(InsuranceClaim.created_at.desc()).limit(100)).scalars().all()
        return [
            {"id": r.id, "claim_number": r.claim_number, "status": r.status, "total_claim": str(r.total_claim), "visit_id": r.visit_id}
            for r in rows
        ]
    ...
    def adjudicate_claim(claim_id, approved_amount, status, notes, tenant_id) -> tuple[bool, dict]:
        ...
        try:
            ...
            pass
        return True, {"id": claim.id, "status": claim.status, "approved_amount": str(claim.approved_amount)}

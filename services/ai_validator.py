...
class AIValidator:
    ...
    def validate_system_data():
        ...
        try:
            ...
            if inactive_rate > 70:
                warnings.append(
                    f'⚠️ تحذير: نسبة المستخدمين غير النشطين عالية جداً ({inactive_rate:.1f}%)'
                )
            ...
            doctors_without_dept = db.session.execute(
                select(func.count()).select_from(User).filter_by(role='doctor', department_id=None)
            ).scalar()
            ...
            old_active_visits = db.session.execute(
                select(func.count())
                .select_from(Visit)
                .filter(
                    Visit.status == 'active',
                    Visit.created_at < datetime.now(UTC) - timedelta(days=1),
                )
            ).scalar()
            ...
            # 6. التحقق من التناسق
            visits_without_doctor = db.session.execute(
                select(func.count()).select_from(Visit).filter_by(doctor_id=None)
            ).scalar()
            if visits_without_doctor > 0:
                errors.append(f'⛔ خطأ: يوجد {visits_without_doctor} زيارة بدون طبيب!')

            visits_without_patient = db.session.execute(
                select(func.count()).select_from(Visit).filter_by(patient_id=None)
            ).scalar()
            ...
            patients_with_future_birth = db.session.execute(
                select(func.count()).select_from(Patient).filter(Patient.birth_date > today)
            ).scalar()

            if patients_with_future_birth > 0:
                errors.append(
                    f'⛔ خطأ: يوجد {patients_with_future_birth} مريض بتاريخ ميلاد في المستقبل!'
                )

            # التحقق من تواريخ الميلاد القديمة جداً (أكثر من 150 سنة)
            very_old_date = date(today.year - 150, today.month, today.day)
            patients_too_old = db.session.execute(
                select(func.count()).select_from(Patient).filter(Patient.birth_date < very_old_date)
            ).scalar()
            ...
            errors.append(f'⛔ خطأ في التحقق: {e!s}')
...
"""
        ...
        return {'report': report, 'status': status, 'color': color, 'validation': validation}

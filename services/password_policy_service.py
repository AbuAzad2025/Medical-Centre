...
class PasswordPolicyService:
    ...
    def validate(self, password: str, user_context: dict | None = None) -> tuple[bool, list[str]]:
        """
        ...
        if len(password) < cfg['min_length']:
            violations.append(f'Password must be at least {cfg["min_length"]} characters long')
        if len(password) > cfg['max_length']:
            violations.append(f'Password must not exceed {cfg["max_length"]} characters')
        ...
        if user_context:
            ...
            for key in ('username', 'email', 'first_name', 'last_name', 'phone'):
                ...
                if value and len(value) >= 3 and value in lowered:
                    violations.append(f'Password must not contain your {key.replace("_", " ")}')
    ...
    def validate_or_raise(self, password: str, user_context: dict | None = None) -> None:
...
def get_password_policy_service(config: dict | None = None) -> PasswordPolicyService:

...
class ClinicalContextService:
    ...
    def get_visit_context(visit_id: int) -> dict[str, Any]:
        ...
        lab_reqs = (
            db.session.execute(select(LabRequest).filter_by(visit_id=visit_id)).scalars().all()
        )
        rad_reqs = (
            db.session.execute(select(RadiologyRequest).filter_by(visit_id=visit_id))
            .scalars()
            .all()
        )
        prescriptions = (
            db.session.execute(select(Prescription).filter_by(visit_id=visit_id)).scalars().all()
        )
        from models.icd_coding import CodedDiagnosis

        diagnoses = (
            db.session.execute(select(CodedDiagnosis).filter_by(visit_id=visit_id)).scalars().all()
        )

        return {
            'visit': visit.to_dict()
            if hasattr(visit, 'to_dict')
            else {'id': visit.id, 'status': visit.status},
            'patient': patient.to_dict()
            if hasattr(patient, 'to_dict')
            else {'id': patient.id, 'name': patient.full_name},
            'vitals': [
                {
            ...
            'lab_requests': [
                {'id': r.id, 'test': r.test_name, 'status': r.status} for r in lab_reqs
            ],
            'radiology_requests': [
                {'id': r.id, 'test': r.test_name, 'status': r.status} for r in rad_reqs
            ],
    ...
    def get_timeline(visit_id: int) -> list[dict[str, Any]]:
        ...
        events = (
            db.session.execute(
                select(VisitWorkflowEvent)
                .filter_by(visit_id=visit_id)
                .order_by(VisitWorkflowEvent.created_at)
            )
            .scalars()
            .all()
        )

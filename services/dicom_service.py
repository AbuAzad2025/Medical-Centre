...
class DICOMService:
    ...
    @staticmethod
    def get_studies(
        patient_id: int | None = None, modality: str | None = None, limit: int = 200
    ) -> list:
    ...
    def get_instances(series_id: int) -> list:
        ...
        return (
            db.session.execute(select(DICOMInstance).filter_by(series_id=series_id)).scalars().all()
        )
    ...
    def get_pacs_config() -> list:
        ...
        return (
            db.session.execute(select(PACSConfiguration).filter_by(is_active=True)).scalars().all()
        )

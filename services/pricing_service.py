...
class PricingService:
    ...
    def create_service_price(service_data, created_by=None):
        ...
        try:
            service_price = ServicePrice(
                ...
                effective_from=service_data.get('effective_from', datetime.now(UTC)),
            ...
            return {
                'success': True,
                'message': 'تم إنشاء سعر الخدمة بنجاح',
    ...
    def create_doctor_pricing(doctor_id, pricing_data, created_by=None):
        ...
        try:
            doctor_pricing = DoctorPricing(
                ...
                effective_from=pricing_data.get('effective_from', datetime.now(UTC)),
            ...
            return {
                'success': True,
                'message': 'تم إنشاء أسعار الطبيب بنجاح',
    ...
    def update_service_price(service_price_id, update_data):
        ...
        try:
            ...
            service_price.updated_at = datetime.now(UTC)
            if not safe_commit(db.session, error_message='فشل عملية قاعدة البيانات'):
                return {'success': False, 'message': 'تعذر تنفيذ العملية حالياً'}
    ...
    def get_pricing_summary(department_id=None, start_date=None, end_date=None):
        ...
        try:
            ...
            if department_id:
                doctor_pricing_query = doctor_pricing_query.filter(
            ...
            # متوسط الأسعار
            avg_service_price = (
                sum(sp.base_price for sp in service_prices) / total_services
                if total_services > 0
                else 0
            )
            avg_consultation_price = (
                sum(dp.consultation_price for dp in doctor_pricing) / total_doctors
                if total_doctors > 0
                else 0
            )
        ...
        try:
            ...
            if visit_data.get('lab_tests'):
                for lab_test_id in visit_data['lab_tests']:
                    ...
                    if lab_test:
                        ...
                        service_name = (
                            getattr(lab_test, 'name', None)
                            or getattr(lab_test, 'name_ar', None)
                            or getattr(lab_test, 'test_name', None)
                        )
                        payment_method = visit_data.get('payment_method', 'cash')
                        lab_price = 0.0
                        if service_name:
                            lab_price = float(
                                PricingService.get_service_price(
                                    service_name, 'lab_test', payment_method
                                )
                                or 0.0
                            )
                        else:
            ...
            if visit_data.get('radiology_tests'):
                for radiology_test_id in visit_data['radiology_tests']:
                    ...
                    if radiology_test:
                        service_name = (
                            getattr(radiology_test, 'name', None)
                            or getattr(radiology_test, 'name_ar', None)
                            or getattr(radiology_test, 'study_uid', None)
                        )
                        payment_method = visit_data.get('payment_method', 'cash')
                        radiology_price = 0.0
                        if service_name:
                            radiology_price = float(
                                PricingService.get_service_price(
                                    service_name, 'radiology_scan', payment_method
                                )
                                or 0.0
                            )
                        else:
        ...
        try:
            ...
            for dept in all_depts:
                ...
                existing = (
                    db.session.execute(
                        select(User)
                        .filter_by(role='doctor', department_id=dept.id, is_active=True)
    ...
    def seed_technicians():
        try:
            created = 0
            lab_dept = (
                db.session.execute(select(Department).filter_by(name='Lab')).scalars().first()
            )
            rad_dept = (
                db.session.execute(select(Department).filter_by(name='Radiology')).scalars().first()
            )
            lab_user = (
                db.session.execute(select(User).filter_by(username='lab_tech')).scalars().first()
            )
            if not lab_user:
                lab_user = User(
                    username='lab_tech',
                    email='lab_tech@example.com',
                    full_name='فني مختبر',
                    role='lab',
                ...
                created += 1
            elif lab_dept and lab_user.department_id != lab_dept.id:
                lab_user.department_id = lab_dept.id
            rad_user = (
                db.session.execute(select(User).filter_by(username='radiology_tech'))
                .scalars()
                .first()
            )
            if not rad_user:
                rad_user = User(
                    ...
                    email='radiology_tech@example.com',
                    full_name='فني أشعة',
                    role='radiology',
                ...
                created += 1
            elif rad_dept and rad_user.department_id != rad_dept.id:
    ...
    def seed_service_master():
        try:
            ...
            for d in defs:
                svc = (
                    db.session.execute(select(ServiceMaster).filter_by(code=d['code']))
                    .scalars()
                    .first()
                )
                if not svc:
                    svc = ServiceMaster(
                        ...
                        insurance_price=d.get('insurance_price'),
                        emergency_price=d.get('emergency_price'),
                        is_active=True,
                    )
                    db.session.add(svc)
                    created += 1
        ...
        try:
            ...
            items = [
                ...
                },
                {
                    ...
                    'service_type': 'lab_test',
                    'base_price': 25.0,
                    'insurance_price': 20.0,
                    'cash_price': 25.0,
                    'vip_price': 50.0,
                },
                {
                    'service_name': 'سكر عشوائي',
                    'service_type': 'lab_test',
                    'base_price': 20.0,
                    'insurance_price': 16.0,
                    'cash_price': 20.0,
                ...
                },
                {
                    ...
                    'service_type': 'lab_test',
                    'base_price': 45.0,
                    'insurance_price': 36.0,
                    'cash_price': 45.0,
                    'vip_price': 90.0,
                },
                {
                    'service_name': 'دهون شاملة',
                    'service_type': 'lab_test',
                    'base_price': 40.0,
                    'insurance_price': 32.0,
                    'cash_price': 40.0,
                    'vip_price': 80.0,
                },
                {
                    'service_name': 'وظائف الكبد',
                    'service_type': 'lab_test',
                    'base_price': 45.0,
                    'insurance_price': 36.0,
                    'cash_price': 45.0,
                ...
                {
                    'service_name': 'دراسات الحديد',
                    'service_type': 'lab_test',
                    'base_price': 55.0,
                    'insurance_price': 44.0,
                    'cash_price': 55.0,
                    'vip_price': 110.0,
                },
                {
                    'service_name': 'فيتامين D',
                    'service_type': 'lab_test',
                    'base_price': 70.0,
                    'insurance_price': 56.0,
                    'cash_price': 70.0,
                    'vip_price': 140.0,
                },
                {
                ...
                {
                    'service_name': 'تروبونين',
                    'service_type': 'lab_test',
                    'base_price': 80.0,
                    'insurance_price': 64.0,
                    'cash_price': 80.0,
                    'vip_price': 160.0,
                },
                {
                    'service_name': 'دي دايمر',
                    'service_type': 'lab_test',
                    'base_price': 60.0,
                    'insurance_price': 48.0,
                    'cash_price': 60.0,
                ...
                },
                {
                    ...
                    'service_type': 'lab_test',
                    'base_price': 65.0,
                    'insurance_price': 52.0,
                    'cash_price': 65.0,
                    'vip_price': 130.0,
                },
                {
                    'service_name': 'CA 19-9',
                    'service_type': 'lab_test',
                    'base_price': 65.0,
                    'insurance_price': 52.0,
                    'cash_price': 65.0,
                ...
                {
                    'service_name': 'أشعة سينية عمود فقري',
                    'service_type': 'radiology_scan',
                    'base_price': 90.0,
                    'insurance_price': 67.5,
                    'cash_price': 90.0,
                    'vip_price': 170.0,
                },
                {
                    'service_name': 'أشعة طرف',
                    'service_type': 'radiology_scan',
                    'base_price': 70.0,
                    'insurance_price': 52.5,
                    'cash_price': 70.0,
                    'vip_price': 130.0,
                },
                {
                    'service_name': 'سونار البطن',
                    'service_type': 'radiology_scan',
                    'base_price': 100.0,
                    'insurance_price': 75.0,
                    'cash_price': 100.0,
                    'vip_price': 200.0,
                },
                {
                    'service_name': 'سونار الحوض',
                    'service_type': 'radiology_scan',
                    'base_price': 90.0,
                    'insurance_price': 67.5,
                    'cash_price': 90.0,
                ...
                {
                    'service_name': 'سونار الحمل',
                    'service_type': 'radiology_scan',
                    'base_price': 120.0,
                    'insurance_price': 90.0,
                    'cash_price': 120.0,
                    'vip_price': 240.0,
                },
                {
                    'service_name': 'سونار الغدة الدرقية',
                    'service_type': 'radiology_scan',
                    'base_price': 90.0,
                    'insurance_price': 67.5,
                    'cash_price': 90.0,
                ...
                {
                    'service_name': 'سونار دوبلر',
                    'service_type': 'radiology_scan',
                    'base_price': 140.0,
                    'insurance_price': 105.0,
                    'cash_price': 140.0,
                    'vip_price': 260.0,
                },
                {
                    'service_name': 'طبقي محوري للرأس',
                    'service_type': 'radiology_scan',
                    'base_price': 250.0,
                    'insurance_price': 187.5,
                    'cash_price': 250.0,
                    'vip_price': 400.0,
                },
                {
                    'service_name': 'طبقي محوري للصدر',
                    'service_type': 'radiology_scan',
                    'base_price': 280.0,
                    'insurance_price': 210.0,
                    'cash_price': 280.0,
                    'vip_price': 420.0,
                },
                {
                    'service_name': 'طبقي محوري للبطن والحوض',
                    'service_type': 'radiology_scan',
                    'base_price': 300.0,
                    'insurance_price': 225.0,
                    'cash_price': 300.0,
                    'vip_price': 450.0,
                },
                {
                    'service_name': 'طبقي محوري شرياني',
                    'service_type': 'radiology_scan',
                    'base_price': 350.0,
                    'insurance_price': 262.5,
                    'cash_price': 350.0,
                    'vip_price': 500.0,
                },
                {
                    'service_name': 'رنين مغناطيسي للدماغ',
                    'service_type': 'radiology_scan',
                    'base_price': 350.0,
                    'insurance_price': 262.5,
                    'cash_price': 350.0,
                ...
                {
                    'service_name': 'رنين مغناطيسي للعمود الفقري',
                    'service_type': 'radiology_scan',
                    'base_price': 400.0,
                    'insurance_price': 300.0,
                    'cash_price': 400.0,
                    'vip_price': 560.0,
                },
                {
                    'service_name': 'رنين مغناطيسي للركبة',
                    'service_type': 'radiology_scan',
                    'base_price': 350.0,
                    'insurance_price': 262.5,
                    'cash_price': 350.0,
                ...
                {
                    'service_name': 'رنين مغناطيسي للكتف',
                    'service_type': 'radiology_scan',
                    'base_price': 350.0,
                    'insurance_price': 262.5,
                    'cash_price': 350.0,
                ...
                {
                    'service_name': 'رنين مغناطيسي للبطن',
                    'service_type': 'radiology_scan',
                    'base_price': 450.0,
                    'insurance_price': 337.5,
                    'cash_price': 450.0,
                    'vip_price': 600.0,
                },
                {
                    'service_name': 'ماموغرام',
                    'service_type': 'radiology_scan',
                    'base_price': 200.0,
                ...
                {
                    'service_name': 'صورة بانورامية للأسنان',
                    'service_type': 'radiology_scan',
                    'base_price': 150.0,
                    'insurance_price': 112.5,
                    'cash_price': 150.0,
                    'vip_price': 250.0,
                },
                {
                    'service_name': 'إيكو قلب',
                    'service_type': 'radiology_scan',
                    'base_price': 200.0,
                    'insurance_price': 150.0,
                    'cash_price': 200.0,
                ...
                {
                    'service_name': 'فلوروسكوبي',
                    'service_type': 'radiology_scan',
                    'base_price': 220.0,
                    'insurance_price': 165.0,
                    'cash_price': 220.0,
                    'vip_price': 350.0,
                },
                {
            ...
            for i in items:
                existing = (
                    db.session.execute(
                        select(ServicePrice).filter(
                            and_(
                                ServicePrice.service_name == i['service_name'],
                                ServicePrice.service_type == i['service_type'],
                            )
                        )
                    )
                    .scalars()
            ...
            for doc in docs:
                ...
                if not exists:
                    dp = DoctorPricing(
                        ...
                        follow_up_price=35.0,
                        emergency_price=100.0,
                        vip_price=120.0,
                        is_active=True,
    ...
    def seed_pricing_catalog():
        try:
            ...
            items = [
                {
                    ...
                    'service_name_ar': 'استشارة',
                    'base_price': 50.0,
                    'insurance_coverage': 20.0,
                    'patient_share': 40.0,
                },
                {
                    'service_type': 'lab',
                    'service_name': 'CBC',
                    'service_name_ar': 'تحليل CBC',
                    'base_price': 30.0,
                    'insurance_coverage': 20.0,
                    'patient_share': 24.0,
                },
                {
                    'service_type': 'lab',
                    'service_name': 'Fasting Blood Glucose',
                    'service_name_ar': 'سكر صائم',
                    'base_price': 25.0,
                    'insurance_coverage': 20.0,
                    'patient_share': 20.0,
                },
                {
                    'service_type': 'lab',
                    'service_name': 'Random Blood Glucose',
                    'service_name_ar': 'سكر عشوائي',
                    'base_price': 20.0,
                    'insurance_coverage': 20.0,
                    'patient_share': 16.0,
                },
                ...
                {
                    'service_type': 'lab',
                    'service_name': 'Renal Function Tests',
                    'service_name_ar': 'وظائف الكلى',
                    'base_price': 45.0,
                    'insurance_coverage': 20.0,
                    'patient_share': 36.0,
                },
                ...
                },
                {
                    ...
                    'service_name_ar': 'عامل روماتويدي',
                    'base_price': 40.0,
                    'insurance_coverage': 20.0,
                    'patient_share': 32.0,
                },
                ...
                {
                    'service_type': 'lab',
                    'service_name': 'HIV Antibody',
                    'service_name_ar': 'HIV Ab',
                    'base_price': 60.0,
                    'insurance_coverage': 20.0,
                    'patient_share': 48.0,
                },
                ...
                {
                    'service_type': 'lab',
                    'service_name': 'Blood Group and Rh',
                    'service_name_ar': 'فصيلة الدم وRh',
                    'base_price': 20.0,
                    'insurance_coverage': 20.0,
                    'patient_share': 16.0,
                },
                {
                    'service_type': 'lab',
                    'service_name': 'Electrolytes',
                    'service_name_ar': 'الكتروليتات',
                    'base_price': 30.0,
                    'insurance_coverage': 20.0,
                    'patient_share': 24.0,
                },
                ...
                {
                    'service_type': 'lab',
                    'service_name': 'Phosphate',
                    'service_name_ar': 'فوسفات',
                    'base_price': 25.0,
                    'insurance_coverage': 20.0,
                    'patient_share': 20.0,
                },
                {
                    'service_type': 'lab',
                    'service_name': 'Troponin',
                    'service_name_ar': 'تروبونين',
                    'base_price': 80.0,
                    'insurance_coverage': 20.0,
                    'patient_share': 64.0,
                },
                {
                    'service_type': 'lab',
                    'service_name': 'D-Dimer',
                    'service_name_ar': 'دي دايمر',
                    'base_price': 60.0,
                    'insurance_coverage': 20.0,
                    'patient_share': 48.0,
                },
                ...
                {
                    'service_type': 'lab',
                    'service_name': 'Urine Culture',
                    'service_name_ar': 'زرع بول',
                    'base_price': 60.0,
                    'insurance_coverage': 20.0,
                    'patient_share': 48.0,
                },
                {
                    'service_type': 'lab',
                    'service_name': 'Blood Culture',
                    'service_name_ar': 'زرع دم',
                    'base_price': 80.0,
                    'insurance_coverage': 20.0,
                    'patient_share': 64.0,
                },
                {
                    'service_type': 'lab',
                    'service_name': 'Sputum Culture',
                    'service_name_ar': 'زرع بلغم',
                    'base_price': 60.0,
                    'insurance_coverage': 20.0,
                    'patient_share': 48.0,
                },
                {
                    ...
                    'service_name': 'Chest X-ray',
                    'service_name_ar': 'أشعة سينية صدر',
                    'base_price': 80.0,
                    'insurance_coverage': 25.0,
                    'patient_share': 60.0,
                },
                {
                    'service_type': 'radiology',
                    'service_name': 'Abdomen X-ray',
                    'service_name_ar': 'أشعة سينية بطن',
                    'base_price': 80.0,
                    'insurance_coverage': 25.0,
                    'patient_share': 60.0,
                },
                {
                    'service_type': 'radiology',
                    'service_name': 'Pelvis X-ray',
                    'service_name_ar': 'أشعة سينية حوض',
                    'base_price': 80.0,
                    'insurance_coverage': 25.0,
                    'patient_share': 60.0,
                },
                {
                    'service_type': 'radiology',
                    'service_name': 'Spine X-ray',
                    'service_name_ar': 'أشعة سينية عمود فقري',
                    'base_price': 90.0,
                    'insurance_coverage': 25.0,
                    'patient_share': 67.5,
                },
                {
                    'service_type': 'radiology',
                    'service_name': 'Extremity X-ray',
                    'service_name_ar': 'أشعة طرف',
                    'base_price': 70.0,
                    'insurance_coverage': 25.0,
                    'patient_share': 52.5,
                },
                ...
                {
                    'service_type': 'radiology',
                    'service_name': 'Thyroid Ultrasound',
                    'service_name_ar': 'سونار الغدة الدرقية',
                    'base_price': 90.0,
                    'insurance_coverage': 25.0,
                    'patient_share': 67.5,
                },
                ...
                },
                {
                    ...
                    'service_name_ar': 'طبقي محوري للرأس',
                    'base_price': 250.0,
                    'insurance_coverage': 25.0,
                    'patient_share': 187.5,
                },
                {
                    ...
                    'service_name_ar': 'طبقي محوري للصدر',
                    'base_price': 280.0,
                    'insurance_coverage': 25.0,
                    'patient_share': 210.0,
                },
                {
                    ...
                    'service_name_ar': 'طبقي محوري للبطن والحوض',
                    'base_price': 300.0,
                    'insurance_coverage': 25.0,
                    'patient_share': 225.0,
                },
                {
                    'service_type': 'radiology',
                    'service_name': 'CT Angiography',
                    'service_name_ar': 'طبقي محوري شرياني',
                    'base_price': 350.0,
                    'insurance_coverage': 25.0,
                    'patient_share': 262.5,
                },
                {
                    ...
                    'service_name_ar': 'رنين مغناطيسي للدماغ',
                    'base_price': 350.0,
                    'insurance_coverage': 25.0,
                    'patient_share': 262.5,
                },
                {
                    'service_type': 'radiology',
                    'service_name': 'MRI Spine',
                    'service_name_ar': 'رنين مغناطيسي للعمود الفقري',
                    'base_price': 400.0,
                    'insurance_coverage': 25.0,
                    'patient_share': 300.0,
                },
                {
                    ...
                    'service_name_ar': 'رنين مغناطيسي للركبة',
                    'base_price': 350.0,
                    'insurance_coverage': 25.0,
                    'patient_share': 262.5,
                },
                {
                    ...
                    'service_name_ar': 'رنين مغناطيسي للكتف',
                    'base_price': 350.0,
                    'insurance_coverage': 25.0,
                    'patient_share': 262.5,
                },
                {
                    ...
                    'service_name_ar': 'رنين مغناطيسي للبطن',
                    'base_price': 450.0,
                    'insurance_coverage': 25.0,
                    'patient_share': 337.5,
                },
                ...
                {
                    'service_type': 'radiology',
                    'service_name': 'Dental Panoramic',
                    'service_name_ar': 'صورة بانورامية للأسنان',
                    'base_price': 150.0,
                    'insurance_coverage': 25.0,
                    'patient_share': 112.5,
                },
                ...
                {
                    'service_type': 'radiology',
                    'service_name': 'Fluoroscopy',
                    'service_name_ar': 'فلوروسكوبي',
                    'base_price': 220.0,
                    'insurance_coverage': 25.0,
                    'patient_share': 165.0,
                },
            ...
            for i in items:
                existing = (
                    db.session.execute(
                        select(PricingCatalog).filter(
                            PricingCatalog.service_type == i['service_type'],
                            PricingCatalog.service_name == i['service_name'],
                        )
                    )
                    .scalars()
                    .first()
                ...
                if not existing:
                    pc = PricingCatalog(
                        **i, is_active=True, is_temporary=False, created_by=creator.id
                    )
                    db.session.add(pc)
                    created += 1
            if not safe_commit(db.session, error_message='فشل عملية قاعدة البيانات'):
    ...
    def cleanup_service_prices():
        try:
            all_prices = (
                db.session.execute(
                    select(ServicePrice).order_by(
                        ServicePrice.service_name,
                        ServicePrice.service_type,
                        ServicePrice.created_at.asc(),
                    )
                )
                .scalars()
                .all()
    ...
    def cleanup_pricing_catalog():
        try:
            all_items = (
                db.session.execute(
                    select(PricingCatalog).order_by(
                        PricingCatalog.service_type,
                        PricingCatalog.service_name,
                        PricingCatalog.created_at.asc(),
                    )
                )
                .scalars()
                .all()
    ...
    def cleanup_doctor_pricing():
        try:
            all_dp = (
                db.session.execute(
                    select(DoctorPricing).order_by(
                        DoctorPricing.doctor_id,
                        DoctorPricing.effective_from.asc(),
                        DoctorPricing.created_at.asc(),
                    )
                )
                .scalars()
                .all()
    ...
    def cleanup_users_by_role(max_keep_per_role: int = 2):
        try:
            ...
            for role in roles:
                users = (
                    db.session.execute(select(User).filter_by(role=role).order_by(User.id.asc()))
                    .scalars()
                    .all()
                )
                if len(users) > max_keep_per_role:
                    ...
                    for u in to_deactivate:
                        ...
                        if role == 'doctor':
                            dps = (
                                db.session.execute(select(DoctorPricing).filter_by(doctor_id=u.id))
                                .scalars()
                                .all()
                            )
                            for dp in dps:
    ...
    def cleanup_all(max_keep_per_role: int = 2):
        try:
            ...
            return {
                'success': True,
                'service_prices_removed': r1.get('removed', 0),
                'catalog_removed': r2.get('removed', 0),
    ...
    def purge_users_keep_policy():
        try:
            kept_ids = set()
            dept_map = {
                d.name: d.id for d in db.session.execute(select(Department)).scalars().all()
            }

            def _keep_first(role):
                u = (
                    db.session.execute(
                        select(User).filter_by(role=role, is_active=True).order_by(User.id.asc())
                    )
                    .scalars()
                    .first()
                )
            ...
            def _ensure_doctor_in(dept_name, username, email, full_name):
                ...
                u = (
                    db.session.execute(
                        select(User)
                        .filter_by(role='doctor', department_id=dept_id)
                        .order_by(User.id.asc())
                    )
                    .scalars()
                    .first()
            ...
            def _ensure_staff(role, dept_name, username, email, full_name):
                ...
                u = (
                    db.session.execute(
                        select(User)
                        .filter_by(role=role, department_id=dept_id)
                        .order_by(User.id.asc())
                    )
                    .scalars()
                    .first()
            ...
            _ensure_staff(
                'radiology', 'Radiology', 'radiology_tech', 'radiology_tech@example.com', 'فني أشعة'
            )
            _ensure_doctor_in(
                'Emergency', 'dr_emergency', 'dr_emergency@example.com', 'طبيب الطوارئ'
            )
            ...
            gc_doc = (
                db.session.execute(
                    ...
                    .filter(Department.name == 'General Clinic')
                    .order_by(User.id.asc())
                )
                .scalars()
                .first()
            ...
            to_delete = (
                db.session.execute(select(User).filter(User.id.notin_(kept_ids))).scalars().all()
            )
            deleted = 0
            for u in to_delete:

...
class ManagerService:
        ...
        try:
            return {
                ...
                'total_staff': db.session.execute(
                    select(func.count()).select_from(User).filter(User.role != 'patient')
                ).scalar(),
                ...
                'active_visits': db.session.execute(
                    select(func.count())
                    .select_from(Visit)
                    .filter(Visit.status.in_(['WAITING', 'INPATIENT', 'OBSERVATION']))
                ).scalar(),
            }
        except Exception:
            return {}
    ...
    def get_financial_summary(period: str = 'monthly') -> dict:
        ...
        try:
            total_billed = db.session.execute(
    ...
    def get_department_performance(department_id: int | None = None) -> list:
        ...
        try:
            ...
            results = (
                db.session.execute(
                    query.group_by(Department.name).order_by(func.count(Visit.id).desc())
                )
                .scalars()
                .all()
            )

...
class AuditCleanupService:
    ...
    def _count_old_records(table_name: str, column: str, cutoff: datetime) -> int:
        ...
        try:
            result = db.session.execute(
                text(
                    f"SELECT COUNT(*) FROM {table_name} WHERE {column} < :cutoff"
                ),
                {"cutoff": cutoff},
            ).scalar()
    ...
    def cleanup_table(
        ...
        try:
            ...
            while True:
                deleted = AuditCleanupService._delete_batch(
                    table_name, column, cutoff, batch_size
                )
                if deleted == 0:
                    break
                total_deleted += deleted
                safe_commit(db.session, error_message=f"{table_name} batch commit failed")
                logger.info(f"{table_name}: deleted batch of {deleted} rows (total={total_deleted})")
                if sleep_ms > 0:
            ...
            logger.info(
                f"{table_name}: cleanup complete — {total_deleted} rows deleted"
            )
            return {
                "table": table_name,

...
class LabRequestForm(FormBase, MedicalEntityMixin, StatusMixin, PriorityMixin):
    ...
    request_number = StringField(
        'رقم الطلب',
        validators=[
            DataRequired(message='رقم الطلب مطلوب'),
            Length(max=50, message='رقم الطلب يجب أن يكون أقل من 50 حرف'),
        ],
    )
    lab_test_id = SelectField(
        'فحص المختبر', coerce=int, validators=[DataRequired(message='فحص المختبر مطلوب')]
    )
    ...
    def load_dynamic_choices(self):
        ...
        technicians = (
            db.session.execute(select(User).filter(User.role.in_(['lab', 'admin', 'manager'])))
            .scalars()
            .all()
        )
...
class LabRequestSearchForm(SearchFormBase, DateRangeMixin):
    ...
    def load_dynamic_choices(self):
        ...
        self.assigned_to.choices = [('', 'جميع الفنيين')] + [
            (t.id, t.full_name) for t in technicians
        ]
...
class RadiologyRequestForm(FormBase, MedicalEntityMixin, StatusMixin, PriorityMixin):
    ...
    radiology_test_id = SelectField(
        'فحص الأشعة', coerce=int, validators=[DataRequired(message='فحص الأشعة مطلوب')]
    )
    test_notes = TextAreaField('ملاحظات الفحص', validators=[Optional()])
    ...
    def load_dynamic_choices(self):
        ...
        tests = (
            db.session.execute(select(RadiologyResult).filter_by(status='ACTIVE')).scalars().all()
        )
        ...
        technicians = (
            db.session.execute(
                select(User).filter(User.role.in_(['radiology', 'admin', 'manager']))
            )
            .scalars()
            .all()
        )
...
class TriageForm(FormBase, MedicalEntityMixin):
    ...
    chief_complaint = TextAreaField(
        'الشكوى الرئيسية', validators=[DataRequired(message='الشكوى الرئيسية مطلوبة')]
    )
    ...
    blood_pressure_systolic = IntegerField(
        ...
        validators=[
            Optional(),
            NumberRange(min=50, max=300, message='ضغط الدم الانقباضي يجب أن يكون بين 50 و 300'),
        ],
    )
    blood_pressure_diastolic = IntegerField(
        ...
        validators=[
            Optional(),
            NumberRange(min=30, max=200, message='ضغط الدم الانبساطي يجب أن يكون بين 30 و 200'),
        ],
    )
    heart_rate = IntegerField(
        ...
        validators=[
            Optional(),
            NumberRange(min=30, max=300, message='معدل النبض يجب أن يكون بين 30 و 300'),
        ],
    )
    respiratory_rate = IntegerField(
        ...
        validators=[
            Optional(),
            NumberRange(min=5, max=60, message='معدل التنفس يجب أن يكون بين 5 و 60'),
        ],
    )
    temperature = FloatField(
        ...
        validators=[
            Optional(),
            NumberRange(min=30, max=45, message='درجة الحرارة يجب أن تكون بين 30 و 45'),
        ],
    )
    oxygen_saturation = IntegerField(
        ...
        validators=[
            Optional(),
            NumberRange(min=50, max=100, message='تشبع الأكسجين يجب أن يكون بين 50 و 100'),
        ],
    )
    ...
    consciousness_level = SelectField(
        'مستوى الوعي',
        choices=[
            ('alert', 'واعي'),
            ('confused', 'مشوش'),
    ...
    recommended_action = SelectField(
        'الإجراء الموصى به',
        choices=[
            ('immediate', 'عاجل فوري'),
            ('urgent', 'عاجل'),
    ...
    def validate_blood_pressure_systolic(self, field):
        ...
        if (
            field.data
            and self.blood_pressure_diastolic.data
            and field.data <= self.blood_pressure_diastolic.data
        ):
...
class QueueItemForm(FormBase, MedicalEntityMixin, StatusMixin, PriorityMixin):
    ...
    queue_position = IntegerField(
        'موضع الطابور',
        validators=[
            DataRequired(message='موضع الطابور مطلوب'),
            NumberRange(min=1, message='موضع الطابور يجب أن يكون أكبر من صفر'),
        ],
    )
    estimated_wait_time = IntegerField(
        'الوقت المتوقع للانتظار (دقيقة)',
        validators=[
            Optional(),
            NumberRange(min=0, message='الوقت المتوقع يجب أن يكون أكبر من أو يساوي صفر'),
        ],
    )
    actual_wait_time = IntegerField(
        'الوقت الفعلي للانتظار (دقيقة)',
        validators=[
            Optional(),
            NumberRange(min=0, message='الوقت الفعلي يجب أن يكون أكبر من أو يساوي صفر'),
        ],
    )
    unit_name = SelectField(
        'الوحدة',
        choices=[
            ('reception', 'الاستقبال'),
            ('doctor', 'الطبيب'),
    ...
    def load_dynamic_choices(self):
        ...
        self.assigned_to.choices = [('', 'اختر المستخدم')] + [
...
class WorkflowStepForm(FormBase, StatusMixin):
    ...
    step_name = StringField(
        'اسم الخطوة',
        validators=[
            DataRequired(message='اسم الخطوة مطلوب'),
            Length(max=100, message='اسم الخطوة يجب أن يكون أقل من 100 حرف'),
        ],
    )
    step_order = IntegerField(
        'ترتيب الخطوة',
        validators=[
            DataRequired(message='ترتيب الخطوة مطلوب'),
            NumberRange(min=1, message='ترتيب الخطوة يجب أن يكون أكبر من صفر'),
        ],
    )
    ...
    estimated_duration = IntegerField(
        ...
        validators=[
            Optional(),
            NumberRange(min=1, message='المدة المتوقعة يجب أن تكون أكبر من صفر'),
        ],
    )
...
class WorkflowTransferForm(FormBase, StatusMixin):
    ...
    transfer_reason = TextAreaField(
        'سبب النقل', validators=[DataRequired(message='سبب النقل مطلوب')]
    )
    notes = TextAreaField('ملاحظات إضافية', validators=[Optional()])

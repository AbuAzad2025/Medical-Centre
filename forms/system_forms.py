...
class RoleForm(FormBase, StatusMixin):
    ...
    name = StringField(
        ...
        validators=[
            ...
            Length(max=50, message='اسم الدور يجب أن يكون أقل من 50 حرف'),
        ],
    )
    name_ar = StringField(
        'اسم الدور (عربي)',
        validators=[
            DataRequired(message='اسم الدور بالعربي مطلوب'),
            Length(max=50, message='اسم الدور بالعربي يجب أن يكون أقل من 50 حرف'),
        ],
    )
...
class PermissionAssignmentForm(FormBase, StatusMixin):
    ...
    permission_id = SelectField(
        'الصلاحية', coerce=int, validators=[DataRequired(message='الصلاحية مطلوبة')]
    )
    granted_by = SelectField(
        'مُعطى من قبل', coerce=int, validators=[DataRequired(message='المُعطي مطلوب')]
    )
    ...
    def load_dynamic_choices(self):
        ...
        permissions = (
            db.session.execute(select(Permission).filter_by(is_active=True)).scalars().all()
        )
...
class DepartmentWorkflowConfigForm(FormBase, StatusMixin):
    ...
    department_id = SelectField(
        'القسم', coerce=int, validators=[DataRequired(message='القسم مطلوب')]
    )
    ...
    max_queue_size = IntegerField(
        'الحد الأقصى لحجم الطابور',
        validators=[Optional(), NumberRange(min=1, message='الحد الأقصى يجب أن يكون أكبر من صفر')],
    )
    estimated_processing_time = IntegerField(
        'الوقت المتوقع للمعالجة (دقيقة)',
        validators=[
            Optional(),
            NumberRange(min=1, message='الوقت المتوقع يجب أن يكون أكبر من صفر'),
        ],
    )
    notification_enabled = BooleanField('تفعيل الإشعارات', default=True)
    escalation_time = IntegerField(
        'وقت التصعيد (دقيقة)',
        validators=[
            Optional(),
            NumberRange(min=0, message='وقت التصعيد يجب أن يكون أكبر من أو يساوي صفر'),
        ],
    )
    notes = TextAreaField('ملاحظات', validators=[Optional()])
...
class FileUploadForm(FormBase, FileUploadMixin, StatusMixin):
    ...
    filename = StringField(
        'اسم الملف',
        validators=[
            DataRequired(message='اسم الملف مطلوب'),
            Length(max=255, message='اسم الملف يجب أن يكون أقل من 255 حرف'),
        ],
    )
    original_filename = StringField(
        'الاسم الأصلي',
        validators=[
            DataRequired(message='الاسم الأصلي مطلوب'),
            Length(max=255, message='الاسم الأصلي يجب أن يكون أقل من 255 حرف'),
        ],
    )
    file_size = IntegerField(
        'حجم الملف (بايت)',
        validators=[
            DataRequired(message='حجم الملف مطلوب'),
            NumberRange(min=1, message='حجم الملف يجب أن يكون أكبر من صفر'),
        ],
    )
    mime_type = StringField(
        'نوع الملف',
        validators=[
            DataRequired(message='نوع الملف مطلوب'),
            Length(max=100, message='نوع الملف يجب أن يكون أقل من 100 حرف'),
        ],
    )
    file_hash = StringField(
        'تشفير الملف',
        validators=[Optional(), Length(max=64, message='تشفير الملف يجب أن يكون أقل من 64 حرف')],
    )
    ...
    def load_dynamic_choices(self):
        ...
        self.visit_id.choices = [('', 'اختر الزيارة')] + [
            (v.id, f'زيارة {v.id} - {v.patient.full_name}') for v in visits
        ]
        ...
        self.lab_request_id.choices = [('', 'اختر طلب المختبر')] + [
            (lr.id, f'طلب {lr.id} - {lr.patient.full_name}') for lr in lab_requests
        ]
        ...
        self.radiology_request_id.choices = [('', 'اختر طلب الأشعة')] + [
            (rr.id, f'طلب {rr.id} - {rr.patient.full_name}') for rr in radiology_requests
        ]
...
class BackupSettingsForm(FormBase, StatusMixin):
    ...
    backup_name = StringField(
        ...
        validators=[
            DataRequired(message='اسم النسخة الاحتياطية مطلوب'),
            Length(max=200, message='اسم النسخة الاحتياطية يجب أن يكون أقل من 200 حرف'),
        ],
    )
    backup_type = SelectField(
        'نوع النسخة الاحتياطية',
        choices=[('full', 'كاملة'), ('incremental', 'تزايدية'), ('differential', 'تفاضلية')],
        validators=[DataRequired(message='نوع النسخة الاحتياطية مطلوب')],
    )
    backup_path = StringField(
        'مسار النسخة الاحتياطية',
        validators=[
            DataRequired(message='مسار النسخة الاحتياطية مطلوب'),
            Length(max=500, message='مسار النسخة الاحتياطية يجب أن يكون أقل من 500 حرف'),
        ],
    )
    is_scheduled = BooleanField('مجدولة', default=False)
    schedule_cron = StringField(
        'جدولة CRON',
        validators=[Optional(), Length(max=100, message='جدولة CRON يجب أن تكون أقل من 100 حرف')],
    )
    retention_days = IntegerField(
        'أيام الاحتفاظ',
        validators=[
            Optional(),
            NumberRange(min=1, message='أيام الاحتفاظ يجب أن تكون أكبر من صفر'),
        ],
    )
...
class AuditSearchForm(SearchFormBase, DateRangeMixin):
    ...
    entity_id = IntegerField(
        'معرف الكيان',
        validators=[Optional(), NumberRange(min=1, message='معرف الكيان يجب أن يكون أكبر من صفر')],
    )
...
class FinancialAuditSearchForm(SearchFormBase, DateRangeMixin):
    ...
    amount_from = DecimalField(
        ...
        validators=[
            Optional(),
            NumberRange(min=0, message='المبلغ يجب أن يكون أكبر من أو يساوي صفر'),
        ],
    )
...
class AIAnalyticsConfigForm(FormBase, StatusMixin):
    ...
    config_name = StringField(
        'اسم الإعداد',
        validators=[
            DataRequired(message='اسم الإعداد مطلوب'),
            Length(max=100, message='اسم الإعداد يجب أن يكون أقل من 100 حرف'),
        ],
    )
    ...
    config_value = TextAreaField(
        'قيمة الإعداد (JSON)', validators=[DataRequired(message='قيمة الإعداد مطلوبة')]
    )
    description = TextAreaField('الوصف', validators=[Optional()])
...
class SystemConfigForm(FormBase, StatusMixin):
    ...
    setting_key = StringField(
        'مفتاح الإعداد',
        validators=[
            DataRequired(message='مفتاح الإعداد مطلوب'),
            Length(max=100, message='مفتاح الإعداد يجب أن يكون أقل من 100 حرف'),
        ],
    )
    setting_value = TextAreaField(
        'قيمة الإعداد', validators=[DataRequired(message='قيمة الإعداد مطلوبة')]
    )
...
class UserRoleAssignmentForm(FormBase, StatusMixin):
    ...
    user_id = SelectField(
        'المستخدم', coerce=int, validators=[DataRequired(message='المستخدم مطلوب')]
    )
    role_id = SelectField('الدور', coerce=int, validators=[DataRequired(message='الدور مطلوب')])
    assigned_by = SelectField(
        'مُعيّن من قبل', coerce=int, validators=[DataRequired(message='المُعيّن مطلوب')]
    )
    expires_at = DateField('تاريخ الانتهاء', validators=[Optional()])
    reason = TextAreaField('السبب', validators=[Optional()])
...
class NotificationForm(FormBase, NotificationMixin, StatusMixin):
    ...
    def load_dynamic_choices(self):
        ...
        users = db.session.execute(select(User).filter_by(is_active=True)).scalars().all()
        self.user_id.choices = [(u.id, f'{u.full_name} ({u.role})') for u in users]
        self.sender_id.choices = [('', 'اختر المرسل')] + [
            (u.id, f'{u.full_name} ({u.role})') for u in users
        ]

...
from .base_forms import DateRangeMixin, FormBase, MedicalEntityMixin, SearchFormBase, StatusMixin
...
class DoctorScheduleForm(FormBase, StatusMixin):
    ...
    schedule_date = DateField(
        'تاريخ الجدولة', validators=[DataRequired(message='تاريخ الجدولة مطلوب')]
    )
    ...
    max_patients = IntegerField(
        'الحد الأقصى للمرضى',
        validators=[
            DataRequired(message='الحد الأقصى للمرضى مطلوب'),
            NumberRange(min=1, message='الحد الأقصى للمرضى يجب أن يكون أكبر من صفر'),
        ],
    )
    appointment_duration = IntegerField(
        'مدة الموعد (دقيقة)',
        validators=[
            DataRequired(message='مدة الموعد مطلوبة'),
            NumberRange(min=5, message='مدة الموعد يجب أن تكون 5 دقائق على الأقل'),
        ],
    )
    break_duration = IntegerField(
        ...
        validators=[
            Optional(),
            NumberRange(min=0, message='مدة الاستراحة يجب أن تكون أكبر من أو تساوي صفر'),
        ],
    )
...
class AvailabilityExceptionForm(FormBase, StatusMixin):
    ...
    start_datetime = DateTimeField(
        'وقت البداية', validators=[DataRequired(message='وقت البداية مطلوب')]
    )
    end_datetime = DateTimeField(
        'وقت النهاية', validators=[DataRequired(message='وقت النهاية مطلوب')]
    )
...
class OnlineBookingForm(FormBase, MedicalEntityMixin):
    ...
    appointment_date = DateField(
        'تاريخ الموعد', validators=[DataRequired(message='تاريخ الموعد مطلوب')]
    )
    appointment_time = TimeField(
        'وقت الموعد', validators=[DataRequired(message='وقت الموعد مطلوب')]
    )
    ...
    contact_phone = StringField(
        ...
        validators=[
            ...
            Length(max=20, message='رقم الهاتف يجب أن يكون أقل من 20 رقم'),
        ],
    )
    ...
    def load_dynamic_choices(self):
        ...
        self.preferred_doctor.choices = [('', 'اختر الطبيب')] + [
            (d.id, d.full_name) for d in doctors
        ]
...
class MedicalRecordForm(FormBase, MedicalEntityMixin, StatusMixin):
    ...
    subjective = TextAreaField(
        'الذاتي (Subjective)', validators=[DataRequired(message='الذاتي مطلوب')]
    )
    objective = TextAreaField(
        'الموضوعي (Objective)', validators=[DataRequired(message='الموضوعي مطلوب')]
    )
    assessment = TextAreaField(
        'التقييم (Assessment)', validators=[DataRequired(message='التقييم مطلوب')]
    )
...
class PrescriptionForm(FormBase, MedicalEntityMixin, StatusMixin):
    ...
    prescription_number = StringField(
        'رقم الوصفة',
        validators=[
            DataRequired(message='رقم الوصفة مطلوب'),
            Length(max=50, message='رقم الوصفة يجب أن يكون أقل من 50 حرف'),
        ],
    )
    prescription_date = DateField(
        'تاريخ الوصفة', validators=[DataRequired(message='تاريخ الوصفة مطلوب')]
    )
    ...
    duration = IntegerField(
        'مدة العلاج (أيام)',
        validators=[
            DataRequired(message='مدة العلاج مطلوبة'),
            NumberRange(min=1, message='مدة العلاج يجب أن تكون أكبر من صفر'),
        ],
    )
    refill_allowed = BooleanField('يُسمح بإعادة الصرف', default=False)
    refill_count = IntegerField(
        'عدد المرات المسموح بها',
        validators=[
            Optional(),
            NumberRange(min=0, message='عدد المرات يجب أن يكون أكبر من أو يساوي صفر'),
        ],
    )
    notes = TextAreaField('ملاحظات إضافية', validators=[Optional()])
...
class PrescriptionItemForm(FormBase):
    ...
    prescription_id = SelectField(
        'الوصفة الطبية', coerce=int, validators=[DataRequired(message='الوصفة الطبية مطلوبة')]
    )
    medication_name = StringField(
        'اسم الدواء',
        validators=[
            DataRequired(message='اسم الدواء مطلوب'),
            Length(max=200, message='اسم الدواء يجب أن يكون أقل من 200 حرف'),
        ],
    )
    dosage = StringField(
        'الجرعة',
        validators=[
            DataRequired(message='الجرعة مطلوبة'),
            Length(max=100, message='الجرعة يجب أن تكون أقل من 100 حرف'),
        ],
    )
    frequency = StringField(
        'التكرار',
        validators=[
            DataRequired(message='التكرار مطلوب'),
            Length(max=100, message='التكرار يجب أن يكون أقل من 100 حرف'),
        ],
    )
    quantity = IntegerField(
        ...
        validators=[
            ...
            NumberRange(min=1, message='الكمية يجب أن تكون أكبر من صفر'),
        ],
    )
    ...
    def load_dynamic_choices(self):
        ...
        prescriptions = (
            db.session.execute(select(Prescription).filter_by(status='ACTIVE')).scalars().all()
        )
        self.prescription_id.choices = [
            (p.id, f'وصفة {p.prescription_number} - {p.patient.full_name}') for p in prescriptions
        ]
...
class FollowUpPlanForm(FormBase, MedicalEntityMixin, StatusMixin):
    ...
    plan_name = StringField(
        'اسم الخطة',
        validators=[
            DataRequired(message='اسم الخطة مطلوب'),
            Length(max=200, message='اسم الخطة يجب أن يكون أقل من 200 حرف'),
        ],
    )

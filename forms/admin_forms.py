...
class DepartmentForm(FlaskForm):
    ...
    current_patients = IntegerField(
        'المرضى الحاليين', validators=[Optional(), NumberRange(min=0, max=1000)]
    )
...
class PasswordResetForm(FlaskForm):
    ...
    new_password = PasswordField(
        'كلمة المرور الجديدة', validators=[DataRequired(), Length(min=6, max=100)]
    )
...
class SystemSettingsForm(FlaskForm):
    ...
    system_name_ar = StringField(
        'اسم النظام بالعربية', validators=[DataRequired(), Length(max=100)]
    )
    ...
    max_patients_per_day = IntegerField(
        'الحد الأقصى للمرضى يومياً', validators=[Optional(), NumberRange(min=1, max=10000)]
    )
    max_appointments_per_doctor = IntegerField(
        'الحد الأقصى للمواعيد لكل طبيب', validators=[Optional(), NumberRange(min=1, max=1000)]
    )
    ...
    session_timeout = IntegerField(
        'مهلة الجلسة (دقيقة)', validators=[Optional(), NumberRange(min=5, max=1440)]
    )
    password_expiry_days = IntegerField(
        'انتهاء كلمة المرور (يوم)', validators=[Optional(), NumberRange(min=0, max=365)]
    )
    max_login_attempts = IntegerField(
        'الحد الأقصى لمحاولات تسجيل الدخول', validators=[Optional(), NumberRange(min=3, max=10)]
    )

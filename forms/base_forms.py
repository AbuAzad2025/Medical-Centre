...
class PaymentMixin:
    ...
    payment_date = DateField(
        'تاريخ الدفع', default=date.today, validators=[DataRequired(message='تاريخ الدفع مطلوب')]
    )
...
class PricingBaseForm(FormBase):
    ...
    base_price = DecimalField(
        'السعر الأساسي',
        validators=[
            DataRequired(message='السعر الأساسي مطلوب'),
            NumberRange(min=0, message='السعر يجب أن يكون أكبر من أو يساوي صفر'),
        ],
    )
    urgent_price = DecimalField(
        'سعر الطوارئ',
        validators=[
            Optional(),
            NumberRange(min=0, message='سعر الطوارئ يجب أن يكون أكبر من أو يساوي صفر'),
        ],
    )
    insurance_price = DecimalField(
        'سعر التأمين',
        validators=[
            Optional(),
            NumberRange(min=0, message='سعر التأمين يجب أن يكون أكبر من أو يساوي صفر'),
        ],
    )
...
class FileUploadMixin:
    ...
    def __init__(self, *args, **kwargs):
        ...
        categories = (
            db.session.execute(select(FileCategory).filter_by(status='ACTIVE')).scalars().all()
        )
        self.file_category.choices = [('', 'اختر فئة الملف')] + [
...
class NotificationMixin:
    ...
    title = StringField(
        ...
        validators=[
            ...
            Length(max=200, message='العنوان يجب أن يكون أقل من 200 حرف'),
        ],
    )
    message = TextAreaField('الرسالة', validators=[DataRequired(message='الرسالة مطلوبة')])
    notification_type = SelectField(
        'نوع الإشعار',
        choices=[('info', 'معلومات'), ('warning', 'تحذير'), ('error', 'خطأ'), ('success', 'نجاح')],
        validators=[DataRequired(message='نوع الإشعار مطلوب')],
    )
    priority = SelectField(
        'الأولوية',
        choices=[('LOW', 'منخفضة'), ('NORMAL', 'عادية'), ('HIGH', 'عالية'), ('URGENT', 'عاجلة')],
        default='NORMAL',
        validators=[DataRequired(message='الأولوية مطلوبة')],
    )

...
class DoctorPricingForm(FlaskForm):
    ...
    # معلومات التسعير
    first_consultation_price = DecimalField(
        'سعر الاستشارة الأولى', validators=[DataRequired(), NumberRange(min=0)]
    )
    ...
    surgery_price = DecimalField(
        'سعر العملية الجراحية', validators=[Optional(), NumberRange(min=0)]
    )

    # معلومات العملة
    currency = SelectField(
        'العملة',
        choices=[('شيكل', 'شيكل'), ('دولار', 'دولار'), ('يورو', 'يورو')],
        validators=[DataRequired()],
    )

    # معلومات إضافية
    consultation_duration = IntegerField(
        'مدة الاستشارة (دقيقة)', validators=[Optional(), NumberRange(min=15, max=480)]
    )
    follow_up_duration = IntegerField(
        'مدة المتابعة (دقيقة)', validators=[Optional(), NumberRange(min=15, max=480)]
    )

    # معلومات الخصم
    discount_percentage = DecimalField(
        'نسبة الخصم (%)', validators=[Optional(), NumberRange(min=0, max=100)]
    )
    ...
    insurance_percentage = DecimalField(
        'نسبة التأمين (%)', validators=[Optional(), NumberRange(min=0, max=100)]
    )
    ...
    def validate_follow_up_price(self, field):
        ...
        if (
            field.data
            and self.first_consultation_price.data
            and field.data > self.first_consultation_price.data
        ):
...
class LabTestPricingForm(FlaskForm):
    ...
    # معلومات التوقيت
    normal_duration = IntegerField(
        'مدة النتيجة العادية (ساعة)', validators=[Optional(), NumberRange(min=1, max=168)]
    )
    urgent_duration = IntegerField(
        'مدة النتيجة العاجلة (ساعة)', validators=[Optional(), NumberRange(min=1, max=168)]
    )
...
class PricingSearchForm(FlaskForm):
    ...
    currency = SelectField(
        'العملة',
        choices=[('', 'جميع العملات'), ('شيكل', 'شيكل'), ('دولار', 'دولار'), ('يورو', 'يورو')],
        validators=[Optional()],
    )
    ...
    insurance_coverage = SelectField(
        'يغطي التأمين',
        choices=[('', 'جميع الحالات'), ('true', 'نعم'), ('false', 'لا')],
        validators=[Optional()],
    )

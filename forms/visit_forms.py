...
class VisitForm(FlaskForm):
    ...
    chief_complaint = TextAreaField(
        'الشكوى الرئيسية', validators=[DataRequired(), Length(max=1000)]
    )
    ...
    medications_prescribed = TextAreaField(
        'الأدوية الموصوفة', validators=[Optional(), Length(max=1000)]
    )
    ...
    payment_status = SelectField(
        'حالة الدفع',
        choices=[('PENDING', 'معلق'), ('PAID', 'مدفوع'), ('PARTIAL', 'جزئي'), ('DEBT', 'دين')],
        validators=[DataRequired()],
    )
...
class VisitSearchForm(FlaskForm):
    ...
    is_emergency = SelectField(
        'حالة الطوارئ',
        choices=[('', 'جميع الحالات'), ('true', 'طوارئ'), ('false', 'عادية')],
        validators=[Optional()],
    )

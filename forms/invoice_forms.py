...
class InvoiceForm(FormBase, MedicalEntityMixin, StatusMixin, PaymentMixin):
    ...
    invoice_number = StringField(
        ...
        validators=[
            ...
            Length(max=50, message='رقم الفاتورة يجب أن يكون أقل من 50 حرف'),
        ],
    )
    total_amount = DecimalField(
        'المبلغ الإجمالي',
        validators=[
            DataRequired(message='المبلغ الإجمالي مطلوب'),
    ...
    paid_amount = DecimalField(
        ...
        validators=[
            Optional(),
            NumberRange(min=0, message='المبلغ المدفوع يجب أن يكون أكبر من أو يساوي صفر'),
        ],
    )
    remaining_amount = DecimalField(
        ...
        validators=[
            Optional(),
            NumberRange(min=0, message='المبلغ المتبقي يجب أن يكون أكبر من أو يساوي صفر'),
        ],
    )
    ...
    insurance_number = StringField(
        'رقم التأمين',
        validators=[Optional(), Length(max=50, message='رقم التأمين يجب أن يكون أقل من 50 حرف')],
    )
    insurance_coverage = DecimalField(
        'نسبة التغطية (%)',
        validators=[
            Optional(),
            NumberRange(min=0, max=100, message='نسبة التغطية يجب أن تكون بين 0 و 100'),
        ],
    )
    ...
    def load_dynamic_choices(self):
        ...
        self.insurance_company_id.choices = [('', 'اختر شركة التأمين')] + [
    ...
    def validate_invoice_number(self, field):
        ...
        existing = (
            db.session.execute(select(Invoice).filter(Invoice.invoice_number == field.data))
            .scalars()
            .first()
        )
        if existing:
...
class InvoiceSearchForm(SearchFormBase, DateRangeMixin):
    ...
    def load_dynamic_choices(self):
        ...
        self.insurance_company_id.choices = [('', 'جميع الشركات')] + [
            (c.id, c.name) for c in companies
        ]
...
class ReceiptForm(FormBase, PaymentMixin):
    ...
    receipt_number = StringField(
        'رقم السند',
        validators=[
            DataRequired(message='رقم السند مطلوب'),
            Length(max=50, message='رقم السند يجب أن يكون أقل من 50 حرف'),
        ],
    )
    invoice_id = SelectField(
        'الفاتورة', coerce=int, validators=[DataRequired(message='الفاتورة مطلوبة')]
    )
    payment_reference = StringField(
        'مرجع الدفع',
        validators=[Optional(), Length(max=100, message='مرجع الدفع يجب أن يكون أقل من 100 حرف')],
    )
    ...
    def load_dynamic_choices(self):
        ...
        def display_for(i):
            patient_name = (
                i.visit.patient.full_name if (i.visit and i.visit.patient) else 'غير محدد'
            )
    ...
    def validate_receipt_number(self, field):
        ...
        existing = (
            db.session.execute(select(Receipt).filter(Receipt.receipt_number == field.data))
            .scalars()
            .first()
        )
        if existing:
...
class RefundForm(FormBase, PaymentMixin):
    ...
    original_payment_id = SelectField(
        'الدفع الأصلي', coerce=int, validators=[DataRequired(message='الدفع الأصلي مطلوب')]
    )
    refund_percentage = DecimalField(
        'نسبة الاسترداد (%)',
        validators=[
            Optional(),
            NumberRange(min=0, max=100, message='نسبة الاسترداد يجب أن تكون بين 0 و 100'),
        ],
    )
    ...
    def load_dynamic_choices(self):
        ...
        self.original_payment_id.choices = [
            (p.id, f'دفع {p.id} - {p.patient.full_name} - {p.amount}') for p in payments
        ]
...
class InsuranceClaimForm(FormBase, MedicalEntityMixin):
    ...
    claim_number = StringField(
        'رقم المطالبة',
        validators=[
            DataRequired(message='رقم المطالبة مطلوب'),
            Length(max=50, message='رقم المطالبة يجب أن يكون أقل من 50 حرف'),
        ],
    )
    insurance_company_id = SelectField(
        'شركة التأمين', coerce=int, validators=[DataRequired(message='شركة التأمين مطلوبة')]
    )
    policy_number = StringField(
        'رقم البوليصة',
        validators=[
            DataRequired(message='رقم البوليصة مطلوب'),
            Length(max=50, message='رقم البوليصة يجب أن يكون أقل من 50 حرف'),
        ],
    )
    claim_amount = DecimalField(
        'مبلغ المطالبة',
        validators=[
            DataRequired(message='مبلغ المطالبة مطلوب'),
            NumberRange(min=0.01, message='مبلغ المطالبة يجب أن يكون أكبر من صفر'),
        ],
    )
    approved_amount = DecimalField(
        ...
        validators=[
            Optional(),
            NumberRange(min=0, message='المبلغ المعتمد يجب أن يكون أكبر من أو يساوي صفر'),
        ],
    )
    claim_date = DateField(
        'تاريخ المطالبة', validators=[DataRequired(message='تاريخ المطالبة مطلوب')]
    )
    ...
    def validate_claim_number(self, field):
        ...
        existing = (
            db.session.execute(
                select(InsuranceClaim).filter(InsuranceClaim.claim_number == field.data)
            )
            .scalars()
            .first()
        )
...
class InsurancePolicyForm(FormBase):
    ...
    patient_id = SelectField(
        'المريض', coerce=int, validators=[DataRequired(message='المريض مطلوب')]
    )
    coverage_percentage = DecimalField(
        'نسبة التغطية (%)',
        validators=[
            DataRequired(message='نسبة التغطية مطلوبة'),
            NumberRange(min=0, max=100, message='نسبة التغطية يجب أن تكون بين 0 و 100'),
        ],
    )
    max_coverage_amount = DecimalField(
        'الحد الأقصى للتغطية',
        validators=[
            Optional(),
            NumberRange(min=0, message='الحد الأقصى يجب أن يكون أكبر من أو يساوي صفر'),
        ],
    )
    start_date = DateField(
        'تاريخ البداية', validators=[DataRequired(message='تاريخ البداية مطلوب')]
    )
...
class InsuranceProviderForm(FormBase):
    ...
    name = StringField(
        'اسم الشركة',
        validators=[
            DataRequired(message='اسم الشركة مطلوب'),
            Length(max=200, message='اسم الشركة يجب أن يكون أقل من 200 حرف'),
        ],
    )
    code = StringField(
        'كود الشركة',
        validators=[
            DataRequired(message='كود الشركة مطلوب'),
            Length(max=50, message='كود الشركة يجب أن يكون أقل من 50 حرف'),
        ],
    )
    contact_person = StringField(
        ...
        validators=[
            Optional(),
            Length(max=100, message='اسم الشخص المسؤول يجب أن يكون أقل من 100 حرف'),
        ],
    )
    phone = StringField(
        'الهاتف',
        validators=[Optional(), Length(max=20, message='رقم الهاتف يجب أن يكون أقل من 20 رقم')],
    )
    email = StringField(
        'البريد الإلكتروني',
        validators=[
            Optional(),
            Length(max=120, message='البريد الإلكتروني يجب أن يكون أقل من 120 حرف'),
        ],
    )

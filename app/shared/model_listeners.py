...
def register_model_listeners():
    ...
    def _visit_after_update(mapper, connection, target):
        try:
            ...
            if insp.attrs.status.history.has_changes():
                old = (
                    insp.attrs.status.history.deleted[0]
                    if insp.attrs.status.history.deleted
                    else None
                )
                new = target.status
                _safe_emit(
    ...
    def _medication_after_update(mapper, connection, target):
        try:
            ...
            if insp.attrs.stock_quantity.history.has_changes():
                old = (
                    insp.attrs.stock_quantity.history.deleted[0]
                    if insp.attrs.stock_quantity.history.deleted
                    else None
                )
                ...
                if new <= target.minimum_stock:
                    _safe_emit(
                        ...
                        medication_id=target.id,
                        stock_quantity=new,
                        min_stock=target.minimum_stock,
                        trade_name=target.trade_name,
                    )
            if insp.attrs.status.history.has_changes():
                _safe_emit(
                    security_event,
                    event_type='medication_status_changed',
                    medication_id=target.id,
                    old_status=insp.attrs.status.history.deleted[0]
                    if insp.attrs.status.history.deleted
                    else None,
                    new_status=target.status,
                )
    ...
    logger.info(
        'Model event listeners registered (visit, lab, radiology, emergency, payment, invoice, prescription, medication, patient, appointment)'
    )

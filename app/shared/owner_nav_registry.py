...
def resolve_owner_nav() -> list[OwnerNavSection]:
    ...
    clients.items.extend(
        [
            ...
            OwnerNavItem(
                'packages', 'حزم SaaS', 'fas fa-box-open', 'owner.owner_packages', '/owner/packages'
            ),
            OwnerNavItem(
                'users', 'إدارة المستخدمين', 'fas fa-users-cog', 'owner.owner_users', '/owner/users'
            ),
        ]
    )
    ...
    product.items.extend(
        [
            OwnerNavItem(
                'plans', 'خطط الاشتراك', 'fas fa-tags', 'owner.owner_plans', '/owner/plans'
            ),
            OwnerNavItem(
                'bundles', 'باقات المنتج', 'fas fa-cubes', 'owner.owner_bundles', '/owner/bundles'
            ),
            OwnerNavItem(
                'billing', 'الفوترة', 'fas fa-coins', 'owner.owner_billing', '/owner/billing'
            ),
        ]
    )
    ...
    ops.items.extend(
        [
            OwnerNavItem(
                'tasks', 'المهام والمشاريع', 'fas fa-tasks', 'owner.owner_tasks', '/owner/tasks'
            ),
            OwnerNavItem(
                'support',
                'الدعم',
                'fas fa-headset',
                'owner.owner_support_tickets',
            ...
            OwnerNavItem(
                ...
                'استخدام الموارد',
                'fas fa-server',
                'owner.owner_resource_usage',
                '/owner/resource-usage',
            ),
            OwnerNavItem(
                'notifications',
                'قواعد الإشعار',
            ...
            OwnerNavItem(
                'webhooks', 'Webhooks', 'fas fa-plug', 'owner.owner_webhooks', '/owner/webhooks'
            ),
            OwnerNavItem(
                'api_keys', 'API Keys', 'fas fa-key', 'owner.owner_api_keys_page', '/owner/api-keys'
            ),
            OwnerNavItem(
                'system_config',
                'إعدادات النظام',
                'fas fa-cogs',
                'owner.owner_system_config',
                '/owner/system-config',
            ),
        ]
    )
    ...
    monitoring.items.extend(
        [
            ...
            OwnerNavItem(
                ...
                'owner.owner_emergency_switches',
                '/owner/emergency-switches',
            ),
        ]
    )

...
class Tenant(db.Model):
    ...
    modules = db.relationship(
        'TenantModule', back_populates='tenant', cascade='all, delete-orphan', lazy='selectin'
    )
...
class TenantSubscriptionHistory(db.Model):
    ...
    )  # ACTIVATE, RENEW, UPGRADE, DOWNGRADE, SUSPEND
...
class PlatformAuditLog(db.Model):
    ...
    action = db.Column(
        db.String(50), nullable=False
    )  # CREATE_TENANT, SUSPEND, RENEW, UPDATE_PLAN, etc.
...
class ResourceUsage(db.Model):
    ...
    def record_snapshot(cls, tenant_id: int) -> 'ResourceUsage':
        ...
        try:
            ...
            total_bytes = db.session.execute(
                select(db.func.coalesce(db.func.sum(FileUpload.file_size), 0)).filter(
        ...
        active_users = db.session.execute(
            select(func.count())
            .select_from(User)
            .filter(User.tenant_id == tenant_id, User.updated_at >= cutoff)
        ).scalar()
        ...
        snapshot = cls(
            ...
            total_users=db.session.execute(
                select(func.count()).select_from(User).filter_by(tenant_id=tenant_id)
            ).scalar(),
            ...
            total_visits=db.session.execute(
                select(func.count()).select_from(Visit).filter_by(tenant_id=tenant_id)
            ).scalar(),
            total_prescriptions=db.session.execute(
                select(func.count()).select_from(Prescription).filter_by(tenant_id=tenant_id)
            ).scalar(),
            total_lab_requests=db.session.execute(
                select(func.count()).select_from(LabRequest).filter_by(tenant_id=tenant_id)
            ).scalar(),
            total_radiology_requests=db.session.execute(
                select(func.count()).select_from(RadiologyRequest).filter_by(tenant_id=tenant_id)
            ).scalar(),
            total_appointments=db.session.execute(
                select(func.count()).select_from(OnlineBooking).filter_by(tenant_id=tenant_id)
            ).scalar(),
            total_invoices=db.session.execute(
                select(func.count()).select_from(Invoice).filter_by(tenant_id=tenant_id)
            ).scalar(),
        )
...
class TenantFeatureFlag(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint('tenant_id', 'feature_key', name='uq_tenant_feature'),)
...
_PRODUCT_PROFILE_SEED: dict[str, dict] = {
    ...
    'custom': {
        ...
        'dashboard_route': '/finance/dashboard',
        'description_ar': 'حسب الطلب - الطبقة الأساسية مدمجة (فوترة وتقارير)',
        'bundle_slug': 'custom',
        'max_users': None,
        'max_patients': None,
    },
...
def seed_default_bundles() -> None:
    ...
    bundle_defs = {
        ...
        'diagnostic_center': {
            ...
            'monthly_price': 1399.00,
            'yearly_price': 13999.00,
            'setup_fee': 800.00,
            'modules': ['reception', 'lab', 'radiology', 'billing', 'reporting'],
            'max_users': 10,
            'max_patients': 3000,
        ...
        'custom': {
            'name': 'Custom Bundle',
            'name_ar': 'حزمة مخصصة',
            'description_ar': 'اختر الموديولز التي تناسبك - الطبقة الأساسية مدمجة',
            'monthly_price': 0.00,
            'yearly_price': 0.00,
            'setup_fee': 0.00,
            'modules': ['billing', 'reporting'],
...
def get_bundle_for_profile(profile_code: str) -> ProductBundle | None:
    ...
    return (
        db.session.execute(
            select(ProductBundle).filter_by(profile_code=profile_code, is_active=True)
        )
        .scalars()
        .first()
    )

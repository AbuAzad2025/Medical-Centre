...
class StripeWebhookEvent(db.Model):
    ...
    received_at = db.Column(db.DateTime, default=lambda: datetime.now(UTC), nullable=False)
...
class Package(db.Model):
    ...
    __table_args__ = (
        CheckConstraint(
            "category IN ('bundle', 'addon', 'standalone')", name='chk_package_category'
        ),
    )

    versions = db.relationship(
        'PackageVersion', back_populates='package', lazy='selectin', cascade='all, delete-orphan'
    )
...
class PackageVersion(db.Model):
    ...
    package_id = db.Column(
        db.Integer, db.ForeignKey('packages.id', ondelete='CASCADE'), nullable=False, index=True
    )
    ...
    published_at = db.Column(db.DateTime, nullable=False, default=lambda: datetime.now(UTC))
...
class PackageVersionEntitlement(db.Model):
    ...
    package_version = db.relationship(
        'PackageVersion', back_populates='entitlements', lazy='selectin'
    )
...
class PackageVersionPricing(db.Model):
    ...
    __table_args__ = (
        CheckConstraint(
            "billing_type IN ('monthly', 'yearly')", name='chk_package_version_pricing_billing_type'
        ),
        CheckConstraint('price >= 0', name='chk_package_version_pricing_price_non_negative'),
        CheckConstraint(
            'setup_fee >= 0', name='chk_package_version_pricing_setup_fee_non_negative'
        ),
    )
...
class SubscriptionLine(TenantMixin, db.Model):
    ...
    status = db.Column(
        db.String(20), nullable=False, default=SubscriptionLineStatus.SCHEDULED, index=True
    )
    billing_type = db.Column(db.String(10), nullable=False, index=True)
    ...
    __table_args__ = (
        CheckConstraint("line_type IN ('base', 'addon')", name='chk_subscription_line_type'),
        CheckConstraint(
            "status IN ('scheduled', 'active', 'ended')", name='chk_subscription_line_status'
        ),
        CheckConstraint(
            "billing_type IN ('monthly', 'yearly')", name='chk_subscription_line_billing_type'
        ),
    ...
    entitlement_grants = db.relationship(
        'EntitlementGrant', back_populates='subscription_line', lazy='selectin'
    )
...
class PackageVersionAvailability(db.Model):
    ...
    package_version = db.relationship(
        'PackageVersion', back_populates='availability_records', lazy='selectin'
    )
...
class TenantOverride(TenantMixin, db.Model):
    ...
    __table_args__ = (
        CheckConstraint("override_type IN ('grant', 'revoke')", name='chk_tenant_override_type'),
        Index('idx_tenant_override_tenant_capability', 'tenant_id', 'capability_key'),
    )

    entitlement_grants = db.relationship(
        'EntitlementGrant', back_populates='tenant_override', lazy='selectin'
    )
...
class EnterpriseContract(TenantMixin, db.Model):
    ...
    entitlements = db.relationship(
        ...
        back_populates='enterprise_contract',
        lazy='selectin',
        cascade='all, delete-orphan',
    )
...
class EnterpriseContractEntitlement(db.Model):
    ...
    enterprise_contract = db.relationship(
        'EnterpriseContract', back_populates='entitlements', lazy='selectin'
    )
    entitlement_grants = db.relationship(
        'EntitlementGrant', back_populates='enterprise_contract_entitlement', lazy='selectin'
    )
...
class EntitlementGrant(TenantMixin, db.Model):
    ...
    subscription_line = db.relationship(
        'SubscriptionLine', back_populates='entitlement_grants', lazy='selectin'
    )
    tenant_override = db.relationship(
        'TenantOverride', back_populates='entitlement_grants', lazy='selectin'
    )
...
class TenantEntitlement(TenantMixin, db.Model):
    ...
    __table_args__ = (Index('idx_tenant_entitlement_effective', 'tenant_id', 'is_effective'),)

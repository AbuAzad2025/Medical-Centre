...
class TenantProvisioningService:
        ...
        *,
        ...
        for grant in line.entitlement_grants:
            ...
            if line_effective_to_naive and (
                grant_effective_to_naive is None
                or grant_effective_to_naive < line_effective_to_naive
            ):
    ...
    def cancel_tenant(
        ...
        *,
        ...
        lines = (
            db.session.execute(select(SubscriptionLine).filter_by(tenant_id=tenant_id))
            .scalars()
            .all()
        )
        for line in lines:
    ...
    @classmethod
    def _create_line_grants(cls, line: SubscriptionLine, package_version: PackageVersion) -> None:
        entitlements = (
            db.session.execute(
                select(PackageVersionEntitlement).filter_by(package_version_id=package_version.id)
            )
            .scalars()
            .all()
        )
    ...
    @classmethod
    def _ensure_modules_for_package(cls, tenant_id: int, package_version: PackageVersion) -> None:
        module_names = {
            ent.module_name
            for ent in db.session.execute(
                select(PackageVersionEntitlement).filter_by(package_version_id=package_version.id)
            )
            .scalars()
            .all()
    ...
    def purge_cancelled_tenants(cls) -> int:
        ...
        cutoff = datetime.now(UTC) - timedelta(days=cls.RETENTION_DAYS)
        count = 0
        tenants = (
            db.session.execute(select(Tenant).filter(Tenant.status == TenantStatus.CANCELLED))
            .scalars()
            .all()
        )

...
class NotificationDispatcher:
    ...
    def notify_appointment_confirmed(
        self, phone: str, patient_name: str, date_str: str, time_str: str, doctor_name: str
    ):
        if self._should_send('whatsapp'):
            try:
                self.whatsapp.send_appointment_reminder(
                    phone, patient_name, date_str, time_str, doctor_name
                )
            except Exception as e:
                logger.warning(f'WhatsApp appointment reminder failed: {e}')

    def notify_lab_results_ready(
        self, phone: str, patient_name: str, visit_number: str, login_link: str | None = None
    ):
        if self._should_send('whatsapp'):
            try:
    ...
    def notify_invoice_generated(
        self, phone: str, patient_name: str, amount: str, receipt_link: str | None = None
    ):
        if self._should_send('whatsapp'):
            try:
    ...
    def notify_medication_dispensed(
        self, phone: str, patient_name: str, medication_name: str, dosage: str
    ):
        if self._should_send('whatsapp'):
            try:

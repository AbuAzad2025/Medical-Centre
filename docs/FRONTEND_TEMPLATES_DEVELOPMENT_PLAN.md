...
| **المرحلة 5** | ✅ **~90 صفحة** — `_page_header` معمّم على كل الوحدات (استقبال، طبيب، صيدلية، مختبر، أشعة، طوارئ، تمريض، جودة، مدير، محاسب، سوبر أدمن، حجز، مالك) |
...
تم ترحيل **69 صفحة** إضافية إلى ماكرو `page_header` عبر 5 وكلاء متوازيين، ليصل الإجمالي إلى **~90 صفحة** تغطي كل الوحدات. صافي التغيير: **−1184 سطر** (حذف نمطية مكررة).

**توسعة الماكرو (متوافقة رجعياً):** دعم `id` و`data_bs_toggle` و`data_bs_target` على أزرار الإجراءات — لترحيل رؤوس الصفحات التي تعتمد على modal triggers أو ربط JS عبر `id` دون لمس أي JS (إعادة استخدام تفويض `data-action` في `events.js`).

**حُرّاس انحدار ذاتية الصيانة** في `test_gate_5_page_header.py` (تفحص كل قالب يستورد الماكرو):
1. يستدعي `page_header(` فعلياً ولا يُبقي علامات رأس قديمة.
2. يُترجم (Jinja compile) دون أخطاء صياغة.
3. كل `url_for` فيه يشير إلى endpoint مُسجّل (يمنع BuildError).

**صفحات مُستثناة عمداً:** رأس بنموذج POST محمي بـ CSRF (`reception/view_appointment`)، ولوحات hero متدرّجة بتصميم مميز (`lab/`, `accountant/dashboard_new`)، وصفحات الطباعة/التعريف الثابتة.

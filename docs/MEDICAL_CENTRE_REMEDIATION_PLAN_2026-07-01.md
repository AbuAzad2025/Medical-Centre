...
| --------- | -------- | -------------- | --------------------------- | -------------------- | ------------ | ---------------------- | -------------- | -------- | --------------- |
| **MC-001** | P0 | B-005: `guard_module` returns silently on missing tenant | `FeatureGateService` | Change line 70 from `return` to `abort(403, description="Tenant context required for module access")` | None | ✅ Baseline `medical-remediation-baseline-2026-07-02`; migration head `s1_007_rls_phase4` | `test_guard_module_missing_tenant_aborts_403` — PASSED (21/21) | Revert single line | **Implemented — Stage 1, commit `6d0205d`** |
...
| **MC-003** | P0 | B-002: Reception routes use naked `db.session.get(Visit, id)` | `get_tenant_record()` helper | Replace `db.session.get(Visit, id)` with `get_tenant_record(Visit, id)` in `archive_visit`, `end_visit`, `view_visit`, `edit_visit` | Verified `get_tenant_record()` contract (does not require MC-001 unless actual code dependency proven) | ✅ Baseline `medical-remediation-baseline-2026-07-02`; migration head `s1_007_rls_phase4` | `test_same_tenant_visit_found`, `test_cross_tenant_visit_raises`, `test_missing_visit_raises`, `test_no_tenant_context_skips_check` — PASSED (4/4) | Revert to `db.session.get` | **Implemented — Stage 1, commit `31c47ae`** |
| **MC-004** | P0 | B-004: Billing routes lack visit ownership validation | `GatekeeperService`, `get_tenant_record()` | Add `get_tenant_record(Visit, id)` in `process_payment`, `archive_visit` (finance), `post_gl` before service delegation | Verified `get_tenant_record()` contract if helper is selected; otherwise specify explicit tenant-filtered query behavior | ✅ Baseline `medical-remediation-baseline-2026-07-02`; migration head `s1_007_rls_phase4` | `test_process_payment_rejects_cross_tenant_visit`, `test_post_gl_rejects_cross_tenant_visit`, `test_finance_archive_rejects_cross_tenant_visit`, `test_get_tenant_record_blocks_cross_tenant_visit` — PASSED (4/4) | Remove explicit lookup | **Implemented — Stage 1, commit `76742c0`** |
...
| 2026-07-02 | Stage 1 implementation approved | Owner approved implementation of Tickets A–D (MC-014 WP-3, MC-001, MC-003, MC-004). Explicitly deferred: MC-014 WP-2/WP-4/WP-5, MC-005, MC-009. | N/A |
| 2026-07-02 | MC-014 WP-3 implemented | `PARTIAL` payment no longer allows normal queue entry. Emergency and force-entry exceptions preserved. | N/A |
| 2026-07-02 | MC-001 implemented | `guard_module` now aborts 403 on missing tenant context. Fail-closed behavior verified. | N/A |
| 2026-07-02 | MC-003 implemented | Reception visit paths use `get_tenant_record`. Cross-tenant access blocked without disclosure. | N/A |
| 2026-07-02 | MC-004 implemented | Billing routes enforce visit ownership via `tenant_id` filter or `get_tenant_record` before service delegation. | N/A |
...
34. **Stage 1 implementation initiated (Update 10):** Baseline checkpoint tag `medical-remediation-baseline-2026-07-02` created on commit `e6495fe`. Migration head `s1_007_rls_phase4` verified. Prior uncommitted changes committed as `0ec5fbf`. PLAN updated with Stage 0 baseline and Stage 1 ticket list.
35. **Ticket A — MC-014 WP-3 implemented (Update 10):** `_check_queue_entry_conditions` `PARTIAL` branch removed for normal visits. Commit `1789d56`. Files: `services/queue_management_service.py`, `tests/test_queue_management_service.py`. Tests: 59 passed.
36. **Ticket B — MC-001 implemented (Update 10):** `guard_module` changed from silent `return` to `abort(403)` when `g.current_tenant` is missing. Commit `6d0205d`. Files: `services/feature_gate_service.py`, `tests/integration/test_feature_gating.py`. Tests: 21 passed.

...
EXPECTED_WIDGETS = {
    ...
    "reception": {"queue_live", "visits_today", "appointments_pending"},  # no cash_summary (billing)
    "accountant": {"pending_payments", "finance_overview", "revenue_today"},
}

# Widgets that must NOT leak to other roles
FORBIDDEN_WIDGETS = {
    "platform_owner": {"queue_live", "pharmacy_dispense", "my_queue", "nurse_assigned", "lab_pending", "pending_payments"},
...
def _make_user(role: str, tenant_id: int = 1):
    u = User(username=f"test_{role}_{tenant_id}", email=f"{role}@test.local", full_name=f"Test {role}", role=role, is_active=True, tenant_id=tenant_id)
...
class TestDashboardCleanliness:
    ...
    def test_er_doctor_has_emergency_layout(self, app):
        ...
        with app.test_request_context():
            ...
            widget_ids = {w.id for w in widgets}
            assert "critical_count" in widget_ids or "triage_board" in widget_ids or "emergency_waitlist" in widget_ids
    ...
    def test_reception_no_billing_widget(self, app):
        ...
        with app.test_request_context():
            ...
            widget_ids = {w.id for w in widgets}
            assert "cash_summary" not in widget_ids, "reception should not see cash_summary (billing)"
    ...
    def test_pharmacist_widgets_gated_by_pharmacy(self, app):
        ...
        with app.test_request_context():
            ...
            assert len(widgets) == 0, "pharmacist widgets should be hidden when pharmacy not enabled"
...
class TestRoleIsolation:
    ...
    def test_reception_cannot_access_clinical(self, app, client, test_tenant):
        ...
        u = ensure_test_user(db, test_tenant, username=f"recv_{test_tenant.id}_rbac", role="reception")
        c = app.test_client()
...
class TestNavFiltering:
    ...
    def test_nav_hides_outside_role(self, app, role):
        ...
        with app.test_request_context():
            ...
            with mock.patch("services.permission_scope_service.PermissionScopeService") as mock_ps:
                ...
                hrefs = []
                for section in nav:
                    items = getattr(section, "items", []) or section.get("items", []) if isinstance(section, dict) else getattr(section, "items", [])
                    for item in items:
    ...
    def test_mobile_nav_platform_owner_normalized(self, app):
        ...
        with app.test_request_context():
            ...
            assert any("owner" in str(h) or "super-admin" in str(h) for h in hrefs), f"platform_owner mobile nav incorrect: {hrefs}"
    ...
    def test_doctor_mobile_no_reception_patients(self, app):
        ...
        with app.test_request_context():
            ...
            assert "reception.patients" not in hrefs and "/reception/patients" not in hrefs, "doctor should not see reception.patients"
...
class TestApiEnforcement:
    ...
    def test_platform_owner_blocked_from_medical_api(self, app, client):
        ...
        for path in ["/doctor/patients", "/lab/requests", "/radiology/requests"]:
            ...
            assert resp.status_code in (302, 403), f"platform_owner should be blocked from {path}, got {resp.status_code}"
        # POST endpoint should also be blocked (405 is also blocked, not 200)
        resp = c.post("/api/lab/requests/1/cancel")
        assert resp.status_code in (302, 403, 405), f"platform_owner should be blocked from POST /api/lab/requests/1/cancel, got {resp.status_code}"

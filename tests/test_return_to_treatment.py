...
class TestReturnToTreatment:
    def test_reception_can_return_completed_visit_to_open(self, app):
        with app.app_context():
            ...
            t = Tenant(
                name='RTT',
                subdomain='rtt-' + uuid.uuid4().hex[:6],
                slug='rtt-' + uuid.uuid4().hex[:6],
            ...
            with app.test_request_context():
                g.tenant_id = t.id
                ok = VisitStateMachineService.return_to_treatment(
                    v, actor=rec, reason='Follow-up needed'
                )
                assert ok
    ...
    def test_doctor_cannot_return_to_treatment(self, app):
        with app.app_context():
            ...
            t = Tenant(
                name='RTT2',
                subdomain='rtt2-' + uuid.uuid4().hex[:6],
                slug='rtt2-' + uuid.uuid4().hex[:6],
                status=TenantStatus.ACTIVE,
                contact_email='r2@t.com',
            )
            _db.session.add(t)
            _db.session.flush()
            g.tenant_id = t.id
    ...
    def test_manager_cannot_return_to_treatment(self, app):
        with app.app_context():
            ...
            t = Tenant(
                name='RTT3',
                subdomain='rtt3-' + uuid.uuid4().hex[:6],
                slug='rtt3-' + uuid.uuid4().hex[:6],
                status=TenantStatus.ACTIVE,
                contact_email='r3@t.com',
            )
            _db.session.add(t)
            _db.session.flush()
            g.tenant_id = t.id
    ...
    def test_non_completed_visit_cannot_return(self, app):
        with app.app_context():
            ...
            t = Tenant(
                name='RTT4',
                subdomain='rtt4-' + uuid.uuid4().hex[:6],
                slug='rtt4-' + uuid.uuid4().hex[:6],
                status=TenantStatus.ACTIVE,
                contact_email='r4@t.com',
            )
            _db.session.add(t)
            _db.session.flush()
            g.tenant_id = t.id
    ...
    def test_return_to_treatment_requires_actor(self, app):
        with app.app_context():
            ...
            t = Tenant(
                name='RTT5',
                subdomain='rtt5-' + uuid.uuid4().hex[:6],
                slug='rtt5-' + uuid.uuid4().hex[:6],
                status=TenantStatus.ACTIVE,
                contact_email='r5@t.com',
            )
            _db.session.add(t)
            _db.session.flush()
            g.tenant_id = t.id
    ...
    def test_add_service_does_not_change_visit_status(self, app, test_tenant, client, login_as):
        ...
        _db.session.commit()
        svc = ServiceMaster(
            code='CATALOG-RTT-' + uuid.uuid4().hex[:6].upper(),
            name='RTT Service',
            category='lab',
            base_price=50,
            is_active=True,

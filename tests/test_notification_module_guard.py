...
def _count(role):
    return db.session.execute(
        select(func.count()).select_from(Notification).filter_by(recipient_role=role)
    ).scalar()
...
def test_enabled_module_notification_is_sent(saas, rollback_db):
    before = _count('pharmacist')
    NotificationService.send_notification(recipient_role='pharmacist', title='t', message='m')
    after = _count('pharmacist')
...
def test_unmapped_role_always_delivered(saas, rollback_db):
    ...
    NotificationService.send_notification(recipient_role='manager', title='t', message='m')

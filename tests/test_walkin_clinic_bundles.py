...
def test_standalone_clinic_bundle_exists(app):
    ...
    with app.app_context():
        bundle = db.session.execute(select(ProductBundle).filter_by(slug="walkin_clinic")).scalars().first()
...
def test_standalone_clinic_activation(app):
    ...
    _seed_bundles_if_empty()
        ...
        with app.app_context():
            t = Tenant(slug=f"walkin-iso-{int(datetime.now(dt_timezone.utc).timestamp())}", name='Walk-in', contact_email='w@test.local', status=TenantStatus.ACTIVE, product_profile_code='walkin_clinic')
        db.session.add(t)
        db.session.commit()

...
from app.shared.enums import InvoiceStatus, StockMovementType
...
def test_dispense_updates_quantity_and_stock(patch_db_session):
    ...
    pi = SimpleNamespace(
        id=100, medication_id=1, quantity=10, dispensed_quantity=0, dispensed_at=None
    )
    session = patch_db_session()
...
def test_dispense_over_prescribed_raises(patch_db_session):
    pi = SimpleNamespace(
        id=100, medication_id=1, quantity=10, dispensed_quantity=8, dispensed_at=None
    )
    session = patch_db_session()

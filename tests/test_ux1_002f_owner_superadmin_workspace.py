...
def owner_user(app, test_tenant):
    u = db.session.execute(select(User).filter_by(username='owner_test_ux1', tenant_id=test_tenant.id)).scalars().first()
    if not u:
        u = User(
...
def superadmin_user(app, test_tenant):
    u = db.session.execute(select(User).filter_by(username='superadmin_test_ux1', tenant_id=test_tenant.id)).scalars().first()
    if not u:
        u = User(

...
RE_INVISIBLE_TEXT = re.compile(
    r'class="[^"]*(?:text-black|text-gray-900|text-slate-900)[^"]*"', re.I
)
...
class TestFrontendAssetQuality:
        ...
        for tmpl in _all_templates():
            ...
            for block_name in ('extra_css', 'extra_js', 'styles', 'scripts', 'css', 'js', 'head'):
                if block_name in blocks:
                    ...
                    has_asset = bool(
                        RE_LINK_CSS.search(body) or RE_SCRIPT_SRC.search(body) or 'url_for' in body
                    )
    ...
    def test_no_hardcoded_dark_contrast_collisions(self):
        ...
        assert not collisions, (
            'Potential contrast collisions (dark container + dark text):\n'
            + '\n'.join(collisions[:50])
        )

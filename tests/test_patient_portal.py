...
def portal_user(app, test_tenant):
    u = (
        db.session.execute(select(User).filter_by(username='portal_user', tenant_id=test_tenant.id))
        .scalars()
        .first()
    )
    if not u:
...
class TestPatientPortalDashboard:
    def test_dashboard_loads_for_linked_patient(
        self, portal_auth_client, portal_patient, monkeypatch
    ):
        from routes import patient_portal

        monkeypatch.setattr(patient_portal, '_get_patient_from_user', lambda: portal_patient)
        resp = portal_auth_client.get('/portal/dashboard')
        assert resp.status_code == 200

    def test_open_invoices_uses_known_statuses_only(
    ...
    def test_total_due_computed_from_total_minus_paid(
        self, portal_auth_client, portal_patient, portal_visit, test_tenant, monkeypatch
    ):
        from routes import patient_portal
    ...
    def test_unread_results_is_zero_when_states_unknown(
        self, portal_auth_client, portal_patient, monkeypatch
    ):
        from routes import patient_portal
    ...
    def test_critical_results_flag_shown(
        ...
        resp = portal_auth_client.get('/portal/dashboard')
        assert resp.status_code == 200
        assert 'متابعة سريرية'.encode() in resp.data

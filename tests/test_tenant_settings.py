...
class TestGetSMSProviderWithTenant:
    ...
    def test_returns_twilio_when_config_complete(self, app, test_tenant):
        ...
        from app.integrations.sms import TwilioSMSProvider, get_sms_provider
    ...
    def test_save_all_sections_together(self, client, manager_auth_client, test_tenant):
        ...
        resp = client.post(
            ...
            json={
                ...
                'lab': {
                    'auto_generate_request_number': True,
                    'request_number_prefix': 'LAB-',
                    'result_decimal_places': 2,
                    'default_result_status': 'PENDING',
                    'critical_result_notification': True,
                    'critical_result_sms': False,
                },
                'radiology': {
                    'auto_generate_request_number': True,
                    'request_number_prefix': 'RAD-',
                    'default_modality': 'CT',
                    'pacs_enabled': False,
                    'pacs_server_url': '',
                    'dicom_aetitle': 'MED_SYS',
                },
            },
        )
        assert resp.status_code == 200

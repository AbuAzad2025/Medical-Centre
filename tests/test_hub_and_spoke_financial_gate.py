...
class TestHubAndSpoke:
    def test_clinical_to_clinical_blocked(self, app, test_tenant):
        ...
        with tenant_test_context(app, t):
            ...
            lab = Department(
                name=f'LabHub-{suf}', name_ar=f'المختبرHub-{suf}', is_active=True, tenant_id=t.id
            )
            radio = Department(
                name=f'RadioHub-{suf}', name_ar=f'الأشعةHub-{suf}', is_active=True, tenant_id=t.id
            )
            ...
            ok, msg = svc.transfer_visit(
                v.id, radio.id, transferred_by=doc_user.id, source='doctor'
            )
            assert ok is False
            assert (
    ...
    def test_reception_to_lab_allowed(self, app, test_tenant):
        ...
        with tenant_test_context(app, t):
            ...
            suf = uuid.uuid4().hex[:6]
            recep = Department(
                name=f'ReceptionHubT-{suf}',
                name_ar=f'الاستقبالHubT-{suf}',
                is_active=True,
                tenant_id=t.id,
            )
            lab = Department(
                name=f'LabHubT-{suf}', name_ar=f'المختبرHubT-{suf}', is_active=True, tenant_id=t.id
            )
            ...
            ok, _msg = svc.transfer_visit(
                v.id, lab.id, transferred_by=recep_user.id, source='reception'
            )
    ...
    def test_lab_order_sets_pending_settlement(self, app, client, test_tenant):
        ...
        db.session.commit()
        visit = Visit(
            tenant_id=test_tenant.id, patient_id=patient.id, doctor_id=doc.id, status='IN_PROGRESS'
        )

...
class TestHubAndSpoke:
    def test_clinical_to_clinical_blocked(self, app, clinical_depts):
        ...
        db.session.add(p)
        db.session.commit()
        v = Visit(tenant_id=t.id, patient_id=p.id, department_id=clinical_depts["lab"].id, status="OPEN")
        db.session.add(v)
        db.session.commit()
        # Use a doctor user
        doc_user = ensure_user(db, t, username="hub_doctor", role="doctor")
        ok, msg = svc.transfer_visit(v.id, clinical_depts["radiology"].id, transferred_by=doc_user.id, source="doctor")
        assert ok is False
    ...
    def test_reception_to_lab_allowed(self, app, clinical_depts):
        ...
        db.session.add(p)
        db.session.commit()
        v = Visit(tenant_id=t.id, patient_id=p.id, department_id=clinical_depts["reception"].id, status="OPEN")
        db.session.add(v)
        db.session.commit()
        recep_user = ensure_user(db, t, username="hub_recep", role="reception")
        ok, msg = svc.transfer_visit(v.id, clinical_depts["lab"].id, transferred_by=recep_user.id, source="reception")
        assert ok is True
    ...
    def test_lab_order_sets_pending_settlement(self, app, client, test_tenant):
        ...
        resp = c.post(f"/doctor/lab-request/{vid}", data={"test_name": "CBC", "notes": "test"})

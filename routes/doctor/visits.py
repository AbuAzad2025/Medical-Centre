...
from app.shared.enums import LabResultStatus, QueueState, VisitState
from models.audit_trail import AuditTrail
from models.medical_record import MedicalRecord
from models.medication import Prescription
from models.visit import Visit
from routes.doctor import _sync_follow_up_request_for_visit, doctor_bp
from routes.doctor.diagnosis import (
    evaluate_clinical_rules,
    get_data_based_recommendations,
    get_standardized_pathways,
)
    ...
    try:
        ...
        ticket = (
            db.session.execute(
                ...
                .filter_by(visit_id=visit_id, department_id=dep_id)
                .order_by(desc(QueueManagement.called_at), QueueManagement.queued_at.asc())
            )
            .scalars()
            .first()
        )
...
def _parse_visit_vital_signs(visit):
    ...
    try:
        ...
        if isinstance(parsed, dict):
            return {
                k: parsed.get(k)
                for k in ('blood_pressure', 'heart_rate', 'temperature', 'respiratory_rate')
            }
    except Exception as e:
        logging.warning(f'Error in {__name__}: {e}')
    return {}
...
def _get_nurse_vital_signs(patient_id):
    try:
        ...
        if latest:
            bp = None
            if (
                latest.blood_pressure_systolic is not None
                or latest.blood_pressure_diastolic is not None
            ):
    ...
    try:
        ...
        radiology_requests = (
            db.session.execute(
                ...
                .filter(RadiologyRequest.visit_id == visit_id)
...
def patient_details(visit_id):
    ...
    try:
        ...
        nurse_latest_vital_signs, nurse_latest_vital_signs_recorded_at = _get_nurse_vital_signs(
            visit.patient_id
        )
        ...
        try:
            ...
            req_ids = [r.id for r in (lab_requests or []) if getattr(r, 'id', None)]
            if req_ids:
                critical_lab_results_count = db.session.execute(
                    select(func.count())
                    .select_from(LabResult)
                    .filter(
                        LabResult.request_id.in_(req_ids),
        ...
        try:
            ...
            req_ids = [r.id for r in (radiology_requests or []) if getattr(r, 'id', None)]
            if req_ids:
                critical_radiology_results_count = db.session.execute(
                    select(func.count())
                    .select_from(RadiologyResult)
                    .filter(
                        RadiologyResult.request_id.in_(req_ids),
        ...
        if visit.notes:
            try:
                ...
                general_notes_count = visit.notes.count('[مذكرة عامة]') + visit.notes.count(
                    '[ملاحظات طبية]'
                )
            except Exception as e:
                logging.warning(f'Error in {__name__}: {e}')
        ...
        clinical_warnings = evaluate_clinical_rules(
            visit, current_prescriptions, structured_vital_signs
        )
...
def end_treatment(visit_id):
    ...
    try:
        ...
        ticket = (
            db.session.execute(
                select(QueueManagement)
                .filter_by(visit_id=visit_id, department_id=visit.department_id)
                .filter(QueueManagement.status == QueueState.IN_PROGRESS)
                .order_by(desc(QueueManagement.started_at))
            )
            .scalars()
            .first()
        )
        if ticket:
            ok, msg = QueueManagementService().complete_treatment(
                ticket.id, completed_by=current_user.id
            )
            if not ok:
                flash(msg, 'warning')
                return redirect(url_for('doctor.patient_queue'))
        VisitStateMachineService.transition(visit, VisitState.COMPLETED, actor=current_user)
        ...
        logging.error(f'Error ending treatment: {e!s}', exc_info=True)

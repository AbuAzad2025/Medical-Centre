...
from routes.emergency import (
    calculate_emergency_performance_score,
    calculate_triage_efficiency,
    ...
    try:
        ...
        priority_analysis = {
            ...
            'urgent': db.session.execute(
                select(func.count())
                .select_from(EmergencyCase)
                .filter(EmergencyCase.severity == 'HIGH')
            ).scalar(),
            'normal': db.session.execute(
                select(func.count())
                .select_from(EmergencyCase)
                .filter(EmergencyCase.severity == 'MODERATE')
            ).scalar(),
            'low': db.session.execute(
                select(func.count())
                .select_from(EmergencyCase)
                .filter(EmergencyCase.severity == 'LOW')
            ).scalar(),
        }
...
def get_critical_alert_system():
    ...
    try:
        ...
        # تنبيهات الموارد
        active_cases = db.session.execute(
            select(func.count())
            .select_from(EmergencyCase)
            .filter(
                EmergencyCase.status.in_(
    ...
    try:
        ...
        workflow_analysis = {
            'waiting': db.session.execute(
                select(func.count())
                .select_from(EmergencyCase)
                .filter(EmergencyCase.status == EmergencyStatus.WAITING)
            ).scalar(),
            'triage': db.session.execute(
                select(func.count())
                .select_from(EmergencyCase)
                .filter(EmergencyCase.status == EmergencyStatus.TRIAGE)
            ).scalar(),
            'resuscitation': db.session.execute(
                select(func.count())
                .select_from(EmergencyCase)
                .filter(EmergencyCase.status == EmergencyStatus.RESUSCITATION)
            ).scalar(),
            'treatment': db.session.execute(
                select(func.count())
                .select_from(EmergencyCase)
                .filter(EmergencyCase.status == EmergencyStatus.TREATMENT)
            ).scalar(),
            'observation': db.session.execute(
                select(func.count())
                .select_from(EmergencyCase)
                .filter(EmergencyCase.status == EmergencyStatus.OBSERVATION)
            ).scalar(),
...
def get_patient_vital_monitoring():
    ...
    try:
        ...
        vital_signs_analysis = {'normal': 0, 'abnormal': 0, 'critical': 0}
...
def get_emergency_resource_management():
    ...
    try:
        ...
        total_staff = db.session.execute(
            select(func.count()).select_from(User).filter(User.role == 'emergency')
        ).scalar()
        active_staff = db.session.execute(
            select(func.count())
            .select_from(User)
            .filter(
                User.role == 'emergency', User.last_login >= datetime.now() - timedelta(hours=24)
            )
        ).scalar()

        # تحليل الأحمال
        today = datetime.now().date()
        today_cases = db.session.execute(
            select(func.count())
            .select_from(EmergencyCase)
...
def get_emergency_analytics():
    ...
    try:
        ...
        completed_cases = db.session.execute(
            select(func.count())
            .select_from(EmergencyCase)
            .filter(EmergencyCase.status == EmergencyStatus.COMPLETED)
        ).scalar()
        ...
        avg_treatment_time = (
            db.session.execute(
                select(
                    func.avg(
                        func.extract('epoch', EmergencyCase.completed_at - EmergencyCase.created_at)
                        / 60
                    )
                ).filter(
                    EmergencyCase.status == EmergencyStatus.COMPLETED,
        ...
        prescriptions_count = db.session.execute(
            select(func.count()).select_from(Prescription).join(EmergencyCase)
        ).scalar()
        lab_requests_count = db.session.execute(
            select(func.count()).select_from(LabRequest).join(EmergencyCase)
        ).scalar()
        radiology_requests_count = db.session.execute(
            select(func.count()).select_from(RadiologyRequest).join(EmergencyCase)
        ).scalar()
        ...
        return {
            ...
            'performance_score': calculate_emergency_performance_score(
...
def get_smart_emergency_recommendations():
    ...
    try:
        ...
        cases_this_week = db.session.execute(
            select(func.count())
            .select_from(EmergencyCase)
            .filter(EmergencyCase.created_at >= week_ago)
        ).scalar()

        cases_last_week = db.session.execute(
            select(func.count())
            .select_from(EmergencyCase)
            .filter(
        ...
        growth_rate = (
            ((cases_this_week - cases_last_week) / cases_last_week * 100)
        ...
        # تحليل الموظفين
        active_emergency_staff = db.session.execute(
            select(func.count())
            .select_from(User)
            .filter(User.role == 'emergency', User.last_login >= datetime.now() - timedelta(days=7))
        ).scalar()

        total_emergency_staff = db.session.execute(
            select(func.count()).select_from(User).filter(User.role == 'emergency')
        ).scalar()

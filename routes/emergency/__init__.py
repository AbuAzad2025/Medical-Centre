...
def get_emergency_time_metrics():
    try:
        ...
        rows = (
            db.session.execute(select(EmergencyCase).filter(EmergencyCase.created_at >= start))
            .scalars()
            .all()
        )
        if not rows:
            return {
                ...
                'door_to_disposition_rate': 0,
                'triage_sla_compliance': 0,
            }
        triage_times = []
        treatment_times = []
        los_times = []
...
def get_emergency_protocols():
    try:
        protocols = [
            {
                'id': 'stroke',
                'name': 'بروتوكول السكتة الدماغية',
                'title': 'بروتوكول السكتة الدماغية',
                'keywords': ['ضعف', 'شلل', 'سكتة', 'stroke'],
                'steps': ['تقييم FAST', 'CT عاجل', 'تفعيل فريق السكتة'],
            },
            {
                'id': 'mi',
                'name': 'بروتوكول MI',
                'title': 'بروتوكول MI',
                'keywords': ['صدر', 'ألم صدري', 'mi', 'heart'],
                'steps': ['ECG خلال 10 دقائق', 'مخبر قلب', 'تحضير قسطرة'],
            },
            {
                'id': 'trauma',
                'name': 'بروتوكول الإصابات',
                'title': 'بروتوكول الإصابات',
                'keywords': ['حادث', 'سقوط', 'جرح', 'trauma'],
                'steps': ['ABC', 'تصوير سريع', 'تحضير غرفة العمليات'],
            },
        ]
        active = (
            db.session.execute(
...
def get_ems_metrics():
    try:
        ...
        ems_cases = db.session.execute(
            select(func.count())
            .select_from(EmergencyCase)
            .filter(EmergencyCase.case_number.like('EMS-%'), EmergencyCase.created_at >= start_7d)
        ).scalar()
        today_responses = db.session.execute(
            select(func.count())
            .select_from(EmergencyCase)
            .filter(EmergencyCase.created_at >= start_today)
        ).scalar()
        ...
        total_cases = db.session.execute(
            select(func.count())
            .select_from(EmergencyCase)
            .filter(EmergencyCase.created_at >= start_7d)
        ).scalar()
            ...
            transport_list.append(
                {
                    ...
                    'patient_name': t.patient.full_name if t.patient else '',
                }
            )
        return {
...
def calculate_triage_efficiency(avg_response_time, priority_analysis):
    ...
    critical_ratio = (
        priority_analysis['critical'] / sum(priority_analysis.values())
        if sum(priority_analysis.values()) > 0
        else 0
    )
...
def calculate_workflow_efficiency(workflow_analysis, avg_total_time):
    ...
    distribution_score = (
        100 - abs(workflow_analysis['triage'] - workflow_analysis['treatment']) / total_cases * 100
    )

...
def _radiology_templates_cfg():
    return (
        db.session.execute(select(SystemConfig).filter_by(config_key='radiology_report_templates'))
        .scalars()
        .first()
    )
...
def _radiology_macros_cfg():
    return (
        db.session.execute(select(SystemConfig).filter_by(config_key='radiology_report_macros'))
        .scalars()
        .first()
    )
    ...
    return [
        {
            'id': secrets.token_hex(8),
            'name': 'Normal',
            'text': 'لا توجد موجودات حادة. ضمن الحدود الطبيعية.',
            'is_active': True,
        },
        {
            'id': secrets.token_hex(8),
            'name': 'Recommend Follow-up',
            'text': 'يوصى بالمتابعة وربط النتائج بالسياق السريري.',
            'is_active': True,
        },
        {
            'id': secrets.token_hex(8),
            'name': 'Limited Study',
            'text': 'الدراسة محدودة بسبب ________. يوصى بإعادة التصوير عند الحاجة.',
            'is_active': True,
        },
    ]
...
def get_radiology_smart_analytics():
    ...
    try:
        ...
        completed_requests = db.session.execute(
            ...
            .filter(RadiologyRequest.status == OrderState.DONE)
        ).scalar()
...
def get_radiology_report_analysis():
    """تحليل التقارير"""
    try:
        total_reports = db.session.execute(
            select(func.count()).select_from(RadiologyResult)
        ).scalar()
        abnormal_findings = db.session.execute(
            select(func.count())
            .select_from(RadiologyResult)
        ...
        last_7 = db.session.execute(
            select(func.count())
            .select_from(RadiologyResult)
            .filter(RadiologyResult.created_at >= (date.today() - timedelta(days=7)))
        ).scalar()
...
def get_radiology_predictive_insights():
    try:
        ...
        weekly_requests = db.session.execute(
            select(func.count())
            .select_from(RadiologyRequest)
            .filter(RadiologyRequest.created_at >= week_start)
        ).scalar()
        monthly_requests = db.session.execute(
            select(func.count())
            .select_from(RadiologyRequest)
            .filter(RadiologyRequest.created_at >= month_start)
        ).scalar()
...
def calculate_imaging_efficiency(avg_time, total_requests):
    ...
    try:
        if avg_time <= 1.5:  # ساعة ونصف أو أقل
            return 95
        if avg_time <= 3:  # 3 ساعات أو أقل
            return 85
        if avg_time <= 4.5:  # 4.5 ساعات أو أقل
            return 75
        return 60
    except (TypeError, ValueError):
        return 0

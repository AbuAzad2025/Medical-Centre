    ...
    try:
            ...
            return jsonify(
                {
                    ...
                    'issue': [
                        {
                            'severity': 'error',
                            'diagnostics': 'تعذر العثور على نتيجة الأشعة المطلوبة',
                        }
                    ],
                }
            ), 404
        req = (
            db.session.execute(
                select(RadiologyRequest).filter(
                    RadiologyRequest.id == res.request_id, RadiologyRequest.tenant_id == g.tenant_id
                )
            )
            .scalars()
            .first()
        ...
        resource = {
            ...
            'code': {
                'coding': [
                    {
                        'system': 'urn:medical-system:modality',
                        'code': (req.modality if req and req.modality else 'RAD'),
                    }
                ],
                'text': code_text,
            },
            ...
            **(
            ...
            + ([{'text': f'StudyUID: {res.study_uid}'}] if res.study_uid else []),
        }
        ...
        return jsonify(
            {
                'resourceType': 'OperationOutcome',
                'issue': [{'severity': 'error', 'diagnostics': 'تعذر تصدير بيانات الأشعة حالياً'}],
            }
        ), 500
    ...
    try:
        ...
        if not res:
            return jsonify(
                {
                    'resourceType': 'OperationOutcome',
                    'issue': [{'severity': 'error', 'diagnostics': 'RadiologyResult not found'}],
                }
            ), 404
        req = (
            db.session.execute(
        ...
        resource = {
            ...
            'category': [
                {
                    'coding': [
                        {'system': 'http://terminology.hl7.org/CodeSystem/v2-0074', 'code': 'RAD'}
                    ]
                }
            ],
            'code': {
                ...
                [
                    {
                        ...
                        'data': base64.b64encode((res.findings or '').encode()).decode(),
                    }
                ]
                if res.findings
                else []
        ...
        return jsonify(
            {
                'resourceType': 'OperationOutcome',
                'issue': [{'severity': 'error', 'diagnostics': 'تعذر تصدير تقرير الأشعة حالياً'}],
            }
        ), 500
...
def api_fhir_imaging_study(result_id):
    ...
    try:
        ...
        resource = {
            ...
            'id': str(res.id),
            **(
                {'identifier': [{'system': 'urn:medical-system:study-uid', 'value': res.study_uid}]}
                if res.study_uid
                else {}
            ),
            'status': 'available',
            ...
            'series': [
                {
                    ...
                    'modality': {
                        'system': 'http://dicom.nema.org/resources/ontology/DCM',
                        'code': (req.modality or 'RAD') if req else 'RAD',
                    },
        ...
        return jsonify(
            {
                'resourceType': 'OperationOutcome',
                'issue': [
                    {'severity': 'error', 'diagnostics': 'تعذر تصدير الدراسة التصويرية حالياً'}
                ],
            }
        ), 500

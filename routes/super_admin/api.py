...
def api_audit_log():
    ...
    try:
        ...
        audit = AuditTrail(
            ...
            + (f'\nraw_action={action}' if safe_action != action else '')
        ...
        logging.exception(f'API audit log error: {e!s}')
...
def api_recent_activities():
    ...
    try:
        ...
        for activity in recent:
            # حساب الوقت النسبي
            time_diff = datetime.now(UTC) - activity.created_at
            ...
            activities.append(
                {
                    ...
                    or f'المستخدم: {activity.user.full_name if activity.user else "غير معروف"}',
                    'time': time_str,
                    'type': activity_type,
                }
            )
        ...
        logging.exception(f'API recent activities error: {e!s}')
...
def api_ai_assistant():
    ...
    try:
        ...
        logging.exception(f'AI Assistant error: {e!s}')
        import traceback

        traceback.print_exc()
        return jsonify(
            {'success': False, 'response': 'تعذر معالجة طلبك حالياً، يرجى المحاولة مرة أخرى'}
        ), 200

...
def manifest():
    ...
    org = (
        ui.get('organization_name')
        if isinstance(ui, dict)
        else getattr(ui, 'organization_name', None)
    )
    primary = (
        ui.get('primary_color') if isinstance(ui, dict) else getattr(ui, 'primary_color', None)
    )
    ...
    return jsonify(
        {
            ...
            'icons': [
                {
                    'src': url_for('static', filename='img/icon-192x192.png'),
                    'sizes': '192x192',
                    'type': 'image/png',
                },
                {
                    'src': url_for('static', filename='img/icon-512x512.png'),
                    'sizes': '512x512',
                    'type': 'image/png',

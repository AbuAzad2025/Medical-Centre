...
def list_medications():
    ...
    try:
        ...
        if stock_status == 'low':
            query = query.filter(
                Medication.stock_quantity <= Medication.minimum_stock, Medication.stock_quantity > 0
            )
        ...
        medications = (
            db.session.execute(
                query.order_by(Medication.trade_name.asc())
                .limit(per_page)
        ...
        logging.error(f'Error listing medications: {e!s}', exc_info=True)
...
def add_medication():
    ...
    if request.method == 'POST':
        try:
            ...
            logging.exception(f'Error adding medication: {e!s}')
    ...
    medication = (
        db.session.execute(
            select(Medication).filter(
                Medication.tenant_id == current_user.tenant_id, Medication.id == medication_id
            )
        )
        .scalars()
        .first()
    ...
    if request.method == 'POST':
        try:
            ...
            medication.trade_name = (
                request.form.get('trade_name') or ''
            ).strip() or medication.trade_name
            medication.scientific_name = (
                request.form.get('scientific_name') or ''
            ).strip() or medication.scientific_name
            ...
            medication.dosage_form = (
                request.form.get('dosage_form') or ''
            ).strip() or medication.dosage_form
            medication.strength = (
                request.form.get('strength') or ''
            ).strip() or medication.strength
            medication.stock_quantity = int(
                request.form.get('stock_quantity', medication.stock_quantity or 0)
            )
            medication.minimum_stock = int(
                request.form.get('minimum_stock', medication.minimum_stock or 10)
            )
            ...
            medication.standard_instructions = (
            ...
            safe_rollback(db.session, error_message='database rollback')
            logging.exception(f'Error editing medication: {e!s}')

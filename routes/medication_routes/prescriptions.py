...
def prescriptions():
    ...
    try:
        ...
        # جلب جميع الروشتات
        prescriptions = db.session.execute(select(Prescription).filter(Prescription.tenant_id == current_user.tenant_id).order_by(Prescription.created_at.desc())).scalars().all()
...
def dispense_prescription(prescription_id):
    try:
        ...
        from models.visit import Visit
        prescription = db.session.execute(select(Prescription).filter(Prescription.id == prescription_id, Prescription.tenant_id == current_user.tenant_id)).scalars().first()
        if not prescription:
        ...
        try:
            ...
            if pairs:
                ...
                for row in rows:
                    a = db.session.execute(select(Medication).filter(Medication.id == row.medication_a_id, Medication.tenant_id == current_user.tenant_id)).scalars().first()
                    b = db.session.execute(select(Medication).filter(Medication.id == row.medication_b_id, Medication.tenant_id == current_user.tenant_id)).scalars().first()

...
def dispense_prescription(prescription_id):
    try:
        ...
        from models.visit import Visit
        prescription = Prescription.query.filter(Prescription.id == prescription_id, Prescription.tenant_id == current_user.tenant_id).first()
        if not prescription:
        ...
        if visit_id:
            visit = Visit.query.filter(Visit.id == visit_id, Visit.tenant_id == current_user.tenant_id).first()
            if visit:
        ...
        try:
            ...
            if pairs:
                ...
                for row in rows:
                    a = Medication.query.filter(Medication.id == row.medication_a_id, Medication.tenant_id == current_user.tenant_id).first()
                    b = Medication.query.filter(Medication.id == row.medication_b_id, Medication.tenant_id == current_user.tenant_id).first()

...
def dashboard():
    ...
    try:
        ...
        # دفعات اليوم
        today_payments = (
            db.session.execute(select(Payment).filter(func.date(Payment.created_at) == today))
            .scalars()
            .all()
        )
        ...
        cash_flow_analysis = _with_defaults(
            get_cash_flow_analysis(), net_cash_flow=0, inflow=0, outflow=0
        )
        ...
        erp_integration = _with_defaults(
            get_erp_integration_status(), status='idle', last_sync=None
        )
        ...
        try:
            ...
            today_currencies = (
                db.session.execute(
                    ...
                    .filter(func.date(Payment.created_at) == today, Payment.currency.isnot(None))
        ...
        try:
            recent = (
                db.session.execute(select(Payment).order_by(Payment.created_at.desc()).limit(10))
                .scalars()
                .all()
            )
            for p in recent:
        ...
        try:
            ...
            if rep and rep.get('success'):
                ...
                debt_alerts['top_overdue'] = sorted(
                    overdue, key=lambda x: float(x.get('remaining_amount') or 0), reverse=True
                )[:10]
                ...
                debt_alerts['top_large'] = sorted(
                    large, key=lambda x: float(x.get('remaining_amount') or 0), reverse=True
                )[:10]

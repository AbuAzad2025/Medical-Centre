...
def visits():
    ...
    total = query.count()
    visits = (
        query.order_by(Visit.created_at.desc()).limit(per_page).offset((page - 1) * per_page).all()
    )
...
def transfer_visit(visit_id):
    ...
    new_department_id = request.form.get('department_id') or (
        request.get_json(silent=True) or {}
    ).get('department_id')
    new_doctor_id = request.form.get('doctor_id') or (request.get_json(silent=True) or {}).get(
        'doctor_id'
    )
...
def create_visit():
    ...
    if request.method == 'POST':
        try:
            ...
            quick_emergency = str(request.form.get('quick_emergency') or '').lower() in [
                '1',
                'true',
                'on',
                'yes',
            ...
            if quick_emergency:
                ...
                try:
                    departments_all = (
                        db.session.execute(select(Department).filter_by(is_active=True))
                        .scalars()
                        .all()
                    )
                    emergency_dept = next(
                        (d for d in departments_all if d.get_type() == 'emergency'), None
                    )
                    if emergency_dept:
            ...
            if selected_tests:
                ...
                for s in services:
                    price = (
                        float(s.insurance_price or s.base_price)
                        if payment_method == 'insurance'
                        else float(s.base_price or 0)
                    )
                    total_cost += price
                ...
                visit_cost = calculate_visit_cost(
                    department_id, doctor_id, visit_type, is_emergency, payment_method
                )
            ...
            try:
                ...
                logging.warning(f'Could not update patient visit counter: {_counter_err!s}')
            ...
            try:
                ...
                logging.warning(f'Could not add to queue: {queue_error!s}')
                flash('حدث خطأ أثناء محاولة إضافة المريض للطابور', 'warning')

            # ========== المرحلة 13: الرسائل والتوجيه ==========
            if is_force_payment:
                flash(
                    'تم إنشاء الزيارة بنجاح. في انتظار موافقة المدير على الدفع القسري.', 'warning'
                )
            ...
            safe_rollback(db.session, error_message='database rollback')
            logging.warning(f'Validation error in create_visit: {ve!s}')
        except Exception as e:
            # أخطاء غير متوقعة
            safe_rollback(db.session, error_message='database rollback')
            flash('تعذر إنشاء الزيارة، يرجى المحاولة مرة أخرى', 'error')
            logging.error(f'Error creating visit: {e!s}', exc_info=True)
    ...
    try:
        ...
        if dept and test_ids_param and dept.get_type() in ['lab', 'radiology']:
            ...
            if tax_type == 'inclusive':
                ...
                breakdown.append(
                    {'item': f'ضريبة ({int(TAX_RATE * 100)}% شاملة)', 'cost': 0}
                )  # للعرض فقط
            ...
            return jsonify(
                {
                    'cost': round(total, 2),
                    'tests_total': round(total, 2),
                    'details': {'breakdown': breakdown},
                }
            )
        cost = calculate_visit_cost(
            department_id, doctor_id, visit_type, is_emergency, payment_method
        )
        ...
        if tax_type == 'inclusive':
            ...
            pricing_details['breakdown'].append(
                {'item': f'ضريبة ({int(TAX_RATE * 100)}%)', 'cost': tax_amount}
            )
...
def get_pricing_details(department_id, doctor_id, visit_type, is_emergency, payment_method):
    ...
    try:
        details = {'service_cost': 0, 'doctor_cost': 0, 'discount': 0, 'total': 0, 'breakdown': []}
        ...
        if department:
            ...
            if service:
                ...
                details['breakdown'].append(
                    {'item': f'خدمة {department.name_ar}', 'cost': details['service_cost']}
                )

        # حساب تكلفة الطبيب
        used_doctor_pricing = False
        if doctor_id:
            doctor_cost = calculate_doctor_cost(
                doctor_id, department_id, visit_type, is_emergency, payment_method
            )
            ...
            if doctor_cost > 0:
                ...
                details['breakdown'] = [
                    {'item': 'رسوم الطبيب (شامل الكشفية)', 'cost': float(doctor_cost)}
                ]
            elif doctor_cost > 0:  # This condition is unreachable but keeping logic structure
                details['breakdown'].append({'item': 'رسوم الطبيب', 'cost': float(doctor_cost)})
        ...
        if visit_type == 'FOLLOW_UP' and not used_doctor_pricing:
            ...
            details['breakdown'].append(
                {'item': 'خصم المراجعة (30%)', 'cost': -details['discount']}
            )
...
def edit_visit(visit_id):
    ...
    departments = (
        db.session.execute(select(Department).filter_by(is_active=True).order_by(Department.name))
        .scalars()
        .all()
    )
    doctors = (
    ...
    return render_template(
        'reception/edit_visit.html', visit=visit, departments=departments, doctors=doctors
    )
    ...
    try:
        ...
        if remaining > 0:
            visit.payment_status = (
                PaymentStatus.PARTIAL if visit.paid_amount > 0 else PaymentStatus.PENDING
            )

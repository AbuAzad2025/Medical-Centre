...
def checkin_online_booking():
    ...
    if booking_id:
        ...
        booking = (
            db.session.execute(select(OnlineBooking).filter_by(booking_reference=booking_reference))
            .scalars()
            .first()
        )
    ...
    booking.confirmed_at = booking.confirmed_at or datetime.now(UTC)
    ...
    try:
        ...
        logging.warning(f'Could not add to queue from online booking: {queue_error!s}')
...
def checkin_appointment(appointment_id: int):
    ...
    appt_date = appointment.starts_at.date() if appointment.starts_at else datetime.now(UTC).date()
    existing_visit = (
        db.session.execute(
            select(Visit)
            .filter(
    ...
    try:
        queue_result = add_patient_to_queue_auto(
        ...
        logging.warning(f'Could not add to queue from appointment: {queue_error!s}')
...
def appointments():
    ...
    appointments = (
        query.order_by(Appointment.starts_at.desc())
        .limit(per_page)
    ...
    doctors = (
        db.session.execute(
            select(User).filter_by(role='doctor', is_active=True).order_by(User.full_name.asc())
        )
        .scalars()
        .all()
    )
    ...
    today_count = (
        db.session.execute(
            select(func.count(Appointment.id)).filter(
                func.date(Appointment.starts_at) == func.current_date()
            )
        ).scalar()
        or 0
    )
...
def follow_ups():
    ...
    followups = (
        db.session.execute(
            query.order_by(
                FollowUpRequest.suggested_date.asc(), FollowUpRequest.created_at.desc()
            ).limit(500)
        )
        .scalars()
        .all()
...
def create_appointment():
    ...
    if request.method == 'POST':
        try:
            ...
            duration_minutes = (
                int(duration_raw) if duration_raw and str(duration_raw).isdigit() else 30
            )
            ...
            follow_up_id = (
                int(follow_up_id) if follow_up_id and str(follow_up_id).isdigit() else None
            )
            if follow_up_id:
                try:
            ...
            if _wants_json():
                return jsonify(
                    {
                        'success': False,
                        'message': 'تعذر إنشاء الموعد، يرجى التحقق من البيانات والمحاولة مرة أخرى',
                    }
                ), 400
...
def edit_appointment(appointment_id):
    ...
    try:
        if appointment.starts_at and appointment.ends_at:
            duration_value = int(
                max(0, (appointment.ends_at - appointment.starts_at).total_seconds()) // 60
            )
    except Exception:
        duration_value = 30

...
def pos_charge():
    if 'billing' not in getattr(g, 'enabled_modules', set()):
        return jsonify(
            {'success': False, 'message': 'وحدة الفوترة غير مفعلة في باقتك الحالية'}
        ), 403
    amount_raw = None
    if request.is_json:
...
def process_payment(visit_id):
    ...
    try:
        ...
        if not existing_invoice:
            invoice = Invoice(
                invoice_number=f'INV-{visit.id}-{int(datetime.now(UTC).timestamp())}',
                visit_id=visit.id,
...
def print_receipt(visit_id):
    ...
    try:
        ...
        try:
            ...
            pricing = None
        if pricing:
            if getattr(visit, 'visit_type', None) in [
                'FIRST',
                'first',
                'CONSULTATION',
                'consultation',
                ...
                if pricing.consultation_price:
                    doctor_fee = Decimal(str(pricing.consultation_price)).quantize(
                        Decimal('0.01'), rounding=ROUND_HALF_UP
                    )
            elif getattr(visit, 'visit_type', None) in ['FOLLOW_UP', 'follow_up']:
                if pricing.follow_up_price:
                    doctor_fee = Decimal(str(pricing.follow_up_price)).quantize(
                        Decimal('0.01'), rounding=ROUND_HALF_UP
                    )
            elif getattr(visit, 'is_emergency', False):
                if pricing.emergency_price:
                    doctor_fee = Decimal(str(pricing.emergency_price)).quantize(
                        Decimal('0.01'), rounding=ROUND_HALF_UP
                    )
        ...
        doctor_fee = min(doctor_fee, total)
        ...
        if getattr(visit, 'visit_type', None) in ['FOLLOW_UP', 'follow_up']:
            ...
            follow_up_discount = (total * Decimal('0.30')).quantize(
                Decimal('0.01'), rounding=ROUND_HALF_UP
            )
    except Exception:
        doctor_fee = None
    ...
    try:
        last_payment = (
            db.session.execute(
                select(Payment).filter_by(visit_id=visit_id).order_by(Payment.created_at.desc())
            )
            .scalars()
            .first()
        )
    ...
    try:
        ...
        survey = (
            db.session.execute(select(PatientSatisfactionSurvey).filter_by(visit_id=visit_id))
            .scalars()
            .first()
        )
        if survey:
...
def get_payment_methods():
    ...
    return [
        ...
        {'value': 'wire', 'label': 'تحويل بنكي', 'fields': ['reference_number', 'bank_name']},
        {
            'value': 'insurance',
            'label': 'تأمين',
            'fields': ['insurance_provider', 'policy_number', 'coverage_percentage'],
        },

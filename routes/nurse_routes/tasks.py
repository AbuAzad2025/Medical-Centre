...
def tasks():
    ...
    try:
        ...
        total = (
            db.session.execute(select(func.count()).select_from(task_query.subquery())).scalar()
            or 0
        )
        pages = (total + per_page - 1) // per_page

        tasks = (
            db.session.execute(task_query.offset((page - 1) * per_page).limit(per_page))
            .scalars()
            .all()
        )

    except Exception as e:
        logging.exception(f'Error loading nurse tasks: {e!s}')
        tasks = []
        active_visits = []
        total = 0
        pages = 0
...
def create_task():
    try:
        ...
        logging.exception(f'Error creating nurse task: {e!s}')
...
def update_task_status(task_id: int):
    try:
        ...
        t = (
            db.session.execute(
                select(Task).filter(Task.id == task_id, Task.tenant_id == current_user.tenant_id)
            )
            .scalars()
            .first()
        )
        ...
        safe_rollback(db.session, error_message='database rollback')
        logging.exception(f'Error updating task status: {e!s}')

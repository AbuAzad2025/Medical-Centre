...
def api_nursing_protocols():
    ...
    for item in items:
        ...
        normalized.append(
            {'id': item.get('id') or f'p_{len(normalized) + 1}', 'title': title, 'steps': steps}
        )
    if not normalized:
...
def reports():
    try:
        ...
        base_q = (
            ...
            .filter(User.role == 'nurse', Task.created_at >= start_dt, Task.created_at <= end_dt)
        )

        total_tasks = (
            db.session.execute(select(func.count()).select_from(base_q.subquery())).scalar() or 0
        )
        completed_tasks = (
            db.session.execute(
                select(func.count()).select_from(
                    base_q.filter(Task.status == TaskState.COMPLETED).subquery()
                )
            ).scalar()
            or 0
        )
        ...
        urgent_tasks = (
            db.session.execute(
                select(func.count()).select_from(
                    base_q.filter(Task.priority == 'urgent').subquery()
                )
            ).scalar()
            or 0
        )
        ...
        by_status = (
            db.session.execute(
                ...
                .join(User, User.id == Task.assigned_to)
                .filter(
                    User.role == 'nurse', Task.created_at >= start_dt, Task.created_at <= end_dt
                )
        ...
        by_department = (
            db.session.execute(
                select(Department.name_ar, func.count(Task.id))
                .join(User, User.id == Task.assigned_to)
        ...
        top_overdue = db.session.execute(
            base_q.filter(
                Task.due_date.isnot(None),
                Task.due_date < now,
                Task.status.in_([TaskState.PENDING, TaskState.IN_PROGRESS, 'on_hold']),

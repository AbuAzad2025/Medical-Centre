...
class ClinicalPathwayStep(TenantMixin, db.Model):
    ...
    pathway_id = db.Column(
        db.Integer,
        db.ForeignKey('clinical_pathways.id', ondelete='CASCADE'),
        nullable=False,
        index=True,
    )
    ...
    description = db.Column(db.Text, nullable=True)
    step_type = db.Column(
        db.String(50), default='TASK'
    )  # TASK, INVESTIGATION, MEDICATION, SURGERY, CONSULTATION, EDUCATION
...
class PatientCarePlan(TenantMixin, db.Model):
    ...
    pathway_id = db.Column(
        db.Integer,
        db.ForeignKey('clinical_pathways.id', ondelete='SET NULL'),
        nullable=True,
        index=True,
    )
    ...
    tasks = db.relationship(
        'CarePlanTask', back_populates='care_plan', lazy='dynamic', cascade='all, delete-orphan'
    )
...
class CarePlanTask(TenantMixin, db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    care_plan_id = db.Column(
        db.Integer,
        db.ForeignKey('patient_care_plans.id', ondelete='CASCADE'),
        nullable=False,
        index=True,
    )

...
class DigitalSignature(TenantMixin, db.Model):
    ...
    )  # PRESCRIPTION, REPORT, CONSENT, DISCHARGE
    ...
    signing_method = db.Column(
        db.String(50), default='PASSWORD'
    )  # PASSWORD, OTP, BIOMETRIC, SMARTCARD
    signed_at = db.Column(db.DateTime, default=lambda: datetime.now(UTC))
...
class SessionLog(TenantMixin, db.Model):
    ...
    login_at = db.Column(db.DateTime, default=lambda: datetime.now(UTC))

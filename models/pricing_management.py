...
class PricingManagement(TenantMixin, db.Model):
    ...
    service_id = db.Column(
        db.Integer,
        db.ForeignKey('service_master.id', ondelete='CASCADE'),
        nullable=False,
        index=True,
    )
...
class PricingRule(TenantMixin, db.Model):
    ...
    def applies_to(self, **kwargs):
        ...
        if self.condition_type in kwargs:
            ...
            if self.condition_operator == 'less_than':
                return float(value) < float(condition_value)
    ...
    def apply_to_price(self, base_price):
        ...
        if self.price_adjustment_type == 'percentage':
            return Decimal(base_price) * (
                Decimal(1) + (Decimal(self.price_adjustment_value) / Decimal(100))
            )

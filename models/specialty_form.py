...
class SpecialtyForm(TenantMixin, db.Model):
    ...
    latest_published_version_id = db.Column(
        db.Integer, db.ForeignKey('specialty_form_versions.id', ondelete='SET NULL'), nullable=True
    )
    ...
    versions = db.relationship(
        ...
        back_populates='form',
        foreign_keys='SpecialtyFormVersion.form_id',
        lazy='selectin',
        cascade='all, delete-orphan',
    )
    latest_published_version = db.relationship(
        'SpecialtyFormVersion', foreign_keys=[latest_published_version_id], lazy='selectin'
    )
...
class SpecialtyFormVersion(TenantMixin, db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    form_id = db.Column(
        db.Integer, db.ForeignKey('specialty_forms.id', ondelete='CASCADE'), nullable=False
    )
    ...
    form = db.relationship(
        'SpecialtyForm', back_populates='versions', foreign_keys=[form_id], lazy='selectin'
    )
    fields = db.relationship(
        ...
        lazy='selectin',
        cascade='all, delete-orphan',
        order_by='SpecialtyFormField.sort_order',
    )
...
class SpecialtyFormField(TenantMixin, db.Model):
    ...
    version_id = db.Column(
        db.Integer, db.ForeignKey('specialty_form_versions.id', ondelete='CASCADE'), nullable=False
    )
    name = db.Column(db.String(120), nullable=False)
...
class SpecialtyFormSubmission(TenantMixin, db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    version_id = db.Column(
        db.Integer, db.ForeignKey('specialty_form_versions.id', ondelete='RESTRICT'), nullable=False
    )

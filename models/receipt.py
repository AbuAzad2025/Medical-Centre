...
import qrcode
from sqlalchemy import CheckConstraint, Index, select
...
class Receipt(TenantMixin, db.Model):
    ...
    payment_id = db.Column(
        db.Integer, db.ForeignKey('payments.id', ondelete='SET NULL'), nullable=True, index=True
    )
    ...
    def generate_receipt_number(self):
        ...
        if not self.receipt_number:
            ...
            last_receipt = (
                db.session.execute(
                    select(Receipt)
                    .filter(Receipt.receipt_number.like(f'RCP-{date_str}-%'))
    ...
    def mark_as_printed(self, user_id):
        ...
        self.printed_at = datetime.now(UTC)
    ...
    def to_dict(self):
        ...
        return {
            ...
            'debt_approved_at': self.debt_approved_at.isoformat()
            if self.debt_approved_at
            else None,

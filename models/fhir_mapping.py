...
class FHIRObservation(TenantMixin, db.Model):
    ...
    internal_source_type = db.Column(
        db.String(50), nullable=False
    )  # LAB_RESULT, VITAL_SIGN, RADIOLOGY, etc.
    ...
    issued_datetime = db.Column(db.DateTime, default=lambda: datetime.now(UTC))
...
class FHIREncounter(TenantMixin, db.Model):
    ...
    )  # PLANNED, IN_PROGRESS, ONLEAVE, FINISHED, CANCELLED
    version = db.Column(db.Integer, default=1)
    is_active = db.Column(db.Boolean, default=True)
...
class FHIRDocumentReference(TenantMixin, db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    fhir_id = db.Column(db.String(100), nullable=False, unique=True, index=True)
    internal_file_id = db.Column(
        db.Integer, db.ForeignKey('file_uploads.id', ondelete='CASCADE'), nullable=True, index=True
    )
    patient_fhir_id = db.Column(db.String(100), nullable=False, index=True)
    resource_json = db.Column(db.Text, nullable=False)

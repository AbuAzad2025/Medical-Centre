...
class LabTestPanelItem(TenantMixin, db.Model):
    ...
    panel_id = db.Column(
        db.Integer,
        db.ForeignKey('lab_test_panels.id', ondelete='CASCADE'),
        nullable=False,
        index=True,
    )
    test_id = db.Column(
        db.Integer,
        db.ForeignKey('lab_test_catalog.id', ondelete='CASCADE'),
        nullable=False,
        index=True,
    )
    sort_order = db.Column(db.Integer, nullable=True, default=0)
    ...
    __table_args__ = (db.Index('idx_lab_panel_item_unique', 'panel_id', 'test_id', unique=True),)

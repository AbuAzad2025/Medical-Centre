...
class LabRequest(TenantMixin, db.Model):
    ...
    status = db.Column(db.String(20), default='REQUESTED', index=True)  # REQUESTED|COLLECTED|RECEIVED|ANALYZING|REVIEWED|APPROVED|IN_PROGRESS|DONE|CANCELLED
    ...
    analyzed_by = db.Column(db.Integer, db.ForeignKey('users.id', ondelete='SET NULL'), nullable=True, index=True)
    ...
    analyst = db.relationship('User', foreign_keys=[analyzed_by], lazy='selectin')

...
class LabRequest(TenantMixin, db.Model):
    ...
    status = db.Column(
        db.String(20), default='REQUESTED', index=True
    )  # REQUESTED|COLLECTED|RECEIVED|ANALYZING|REVIEWED|APPROVED|IN_PROGRESS|DONE|CANCELLED
...
class LabResult(TenantMixin, db.Model):
    ...
    request_id = db.Column(
        db.Integer, db.ForeignKey('lab_requests.id', ondelete='CASCADE'), nullable=False, index=True
    )
    patient_id = db.Column(
        db.Integer, db.ForeignKey('patients.id', ondelete='CASCADE'), nullable=False, index=True
    )

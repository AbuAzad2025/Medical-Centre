...
class OnlineBooking(TenantMixin, db.Model):
    ...
    def generate_booking_reference():
        ...
        while True:
            ...
            if (
                not db.session.execute(select(OnlineBooking).filter_by(booking_reference=ref))
                .scalars()
                .first()
            ):
                return ref
...
class PaymentTransaction(TenantMixin, db.Model):
    ...
    booking_id = db.Column(
        db.Integer,
        db.ForeignKey('online_bookings.id', ondelete='CASCADE'),
        nullable=False,
        index=True,
    )
    ...
    # تواريخ
    initiated_at = db.Column(db.DateTime, default=lambda: datetime.now(UTC))
        ...
        while True:
            ...
            if (
                not db.session.execute(
                    select(PaymentTransaction).filter_by(transaction_reference=ref)
                )
                .scalars()
                .first()
            ):

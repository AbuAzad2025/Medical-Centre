...
class Role(TenantMixin, db.Model):
    ...
    module_permissions = db.relationship(
        'ModulePermission', back_populates='role', cascade='all, delete-orphan'
    )
    department_permissions = db.relationship(
        'DepartmentPermission', back_populates='role', cascade='all, delete-orphan'
    )
...
class RolePermission(TenantMixin, db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    role_id = db.Column(
        db.Integer, db.ForeignKey('roles.id', ondelete='CASCADE'), nullable=False, index=True
    )
    permission_id = db.Column(
        db.Integer, db.ForeignKey('permissions.id', ondelete='CASCADE'), nullable=False, index=True
    )
    ...
    granter = db.relationship(
        'User', foreign_keys=[granted_by], back_populates='granted_role_permissions'
    )
...
def create_default_permissions():
    ...
    permissions = [
        ...
        (
            'backup_create',
            'إنشاء نسخ احتياطية',
            PermissionCategory.BACKUP_RESTORE,
            PermissionLevel.ADMIN,
        ),
        (
            'backup_restore',
...
def assign_super_admin_permissions():
    ...
    super_admin_role = (
        db.session.execute(select(Role).filter_by(name='super_admin')).scalars().first()
    )

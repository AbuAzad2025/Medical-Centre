...
class ServicePrice(TenantMixin, db.Model):
    ...
    )  # consultation, lab_test, radiology_scan, medication, procedure
    ...
    # التواريخ
    effective_from = db.Column(db.DateTime, default=lambda: datetime.now(UTC))
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "service_type IN ('consultation', 'lab_test', 'radiology_scan', 'medication', 'procedure', 'emergency', 'other')",
    ...
    def get_price(self, payment_method='cash'):
        ...
        if payment_method == 'cash' and self.cash_price:
...
class DoctorPricing(TenantMixin, db.Model):
    ...
    doctor = db.relationship(
        'User', foreign_keys=[doctor_id], back_populates='pricing', lazy='selectin'
    )
...
class PricingCatalog(TenantMixin, db.Model):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            name='chk_pricing_insurance_coverage',
        ),
        CheckConstraint('patient_share >= 0', name='chk_pricing_patient_share'),
        CheckConstraint(
            "service_type IN ('consultation', 'lab', 'radiology', 'medication', 'other')",
            name='chk_pricing_service_type',
        ),
    ...
    def get_insurance_coverage_amount(self):
        ...
        return (
            Decimal(self.base_price) * (Decimal(str(self.insurance_coverage)) / Decimal(100))

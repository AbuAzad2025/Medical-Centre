...
class WhatIfScenario(TenantMixin, db.Model):
    ...
    def calculate_projections(self):
        ...
        if (
            self.scenario_type == 'add_doctor'
            and self.baseline_visits_per_day
            and self.param_new_staff_count
        ):
            ...
            self.scenario_type == 'add_beds'
            and self.baseline_visits_per_day
            and self.param_new_bed_count
        ):
            ...
            if self.baseline_revenue_per_day:
                self.projected_revenue_per_day = (
                    float(self.baseline_revenue_per_day) * factor * demand_factor
                )
            if self.baseline_visits_per_day:
        ...
        if self.baseline_visits_per_day and self.projected_visits_per_day:
            increase = (
                float(self.projected_visits_per_day) - self.baseline_visits_per_day
            ) / self.baseline_visits_per_day

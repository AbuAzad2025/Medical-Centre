...
class BarcodeScanLog(TenantMixin, db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    barcode_registry_id = db.Column(
        db.Integer,
        db.ForeignKey('barcode_registry.id', ondelete='CASCADE'),
        nullable=False,
        index=True,
    )
    ...
    scanned_at = db.Column(db.DateTime, default=lambda: datetime.now(UTC))
    ...
    )  # SUCCESS, MISMATCH, NOT_FOUND, EXPIRED

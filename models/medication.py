...
class Medication(TenantMixin, db.Model):
    ...
    is_controlled = db.Column(
        ...
        default=False,
        nullable=False,
        comment='Controlled substance flag (DEA / C-II-V schedule)',
    )
    schedule = db.Column(
        db.String(20),
        nullable=True,
        comment='Controlled substance schedule (II-V), nullable for non-controlled medications',
    )
    ...
    prescription_items = db.relationship(
        'PrescriptionItem', back_populates='medication', lazy='dynamic'
    )
...
class PharmacySale(TenantMixin, db.Model):
    ...
    items = db.relationship(
        'PharmacySaleItem', back_populates='sale', cascade='all, delete-orphan', lazy='selectin'
    )
...
class PharmacyReturn(TenantMixin, db.Model):
    ...
    sale_item_id = db.Column(
        db.Integer,
        db.ForeignKey('pharmacy_sale_items.id', ondelete='CASCADE'),
        nullable=False,
        index=True,
    )
    medication_id = db.Column(
...
class MedicationPurchase(TenantMixin, db.Model):
    ...
    supplier_id = db.Column(
        db.Integer, db.ForeignKey('suppliers.id', ondelete='SET NULL'), nullable=True, index=True
    )
    medication_id = db.Column(

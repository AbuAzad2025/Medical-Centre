...
class User(TenantMixin, UserMixin, db.Model):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "(role ~ '^(admin|super_admin|manager|doctor|er_doctor|nurse|reception|accountant|emergency|lab|radiology|pharmacist|technician|owner|patient|user|receptionist|lab_tech|platform_owner|rl_.*|unknown_role|)$' OR role = '')",
            name='chk_user_role',
        ),
    ...
    updated_at = db.Column(
        db.DateTime, default=db.func.now(), onupdate=db.func.now(), nullable=False, index=True
    )
    ...
    doctor_visits = db.relationship(
        'Visit', foreign_keys='Visit.doctor_id', back_populates='doctor', lazy='selectin'
    )
    ...
    audit_trails = db.relationship(
        'AuditTrail', foreign_keys='AuditTrail.user_id', back_populates='user', lazy='selectin'
    )

    # علاقات سجلات النظام
    system_logs = db.relationship(
        'SystemLog', foreign_keys='SystemLog.user_id', back_populates='user', lazy='selectin'
    )
    ...
    created_backups = db.relationship(
        'Backup', foreign_keys='Backup.created_by', back_populates='creator', lazy='selectin'
    )
    ...
    audit_logs = db.relationship(
        'AuditLog', foreign_keys='AuditLog.user_id', back_populates='user', lazy='selectin'
    )
    ...
    assigned_tasks = db.relationship(
        'Task', foreign_keys='Task.assigned_to', back_populates='assignee', lazy='selectin'
    )
    created_tasks = db.relationship(
        'Task', foreign_keys='Task.assigned_by', back_populates='assigner', lazy='selectin'
    )
    task_comments = db.relationship(
        'TaskComment', foreign_keys='TaskComment.user_id', back_populates='user', lazy='selectin'
    )
    ...
    managed_projects = db.relationship(
        'Project', foreign_keys='Project.project_manager', back_populates='manager', lazy='selectin'
    )
    created_projects = db.relationship(
        'Project', foreign_keys='Project.created_by', back_populates='creator', lazy='selectin'
    )
    project_tasks = db.relationship(
        'ProjectTask', foreign_keys='ProjectTask.added_by', back_populates='adder', lazy='selectin'
    )
    ...
    schedules = db.relationship(
        'StaffWorkSchedule', back_populates='user', cascade='all, delete-orphan', lazy='selectin'
    )
    absences = db.relationship(
        'StaffAbsence', back_populates='user', cascade='all, delete-orphan', lazy='selectin'
    )
    ...
    signed_medical_reports = db.relationship(
        'MedicalReport',
        back_populates='signer',
        foreign_keys='MedicalReport.signed_by',
        lazy='selectin',
    )
    ...
    sent_whatsapp_messages = db.relationship(
        'WhatsAppMessage', back_populates='sent_by_user', lazy='selectin'
    )
    workflow_transfers = db.relationship(
        'WorkflowTransfer', back_populates='transferred_by_user', lazy='selectin'
    )
    workflow_events = db.relationship(
        'VisitWorkflowEvent', back_populates='performer', lazy='selectin'
    )
    ...
    def set_password(self, password: str, **kwargs) -> None:
        ...
        if enforce and user_context is not None:
            try:
                from services.password_policy_service import (
                    PasswordPolicyError,
...
def _create_default_schedule(mapper, connection, target):
    try:
        ...
        logging.getLogger(__name__).warning(
            'Failed to create default schedule for user %s: %s', target.id, exc
        )

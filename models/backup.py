...
class Backup(TenantMixin, db.Model):
    ...
    __table_args__ = (
        CheckConstraint(
            "backup_type IN ('full', 'incremental', 'differential')", name='chk_backup_type'
        ),
        CheckConstraint(
            "backup_status IN ('PENDING', 'IN_PROGRESS', 'COMPLETED', 'FAILED', 'CANCELLED')",
            name='chk_backup_status',
        ),
        CheckConstraint(
            "schedule_type IN ('daily', 'weekly', 'monthly', 'custom')", name='chk_schedule_type'
        ),
    ...
    creator = db.relationship(
        'User', foreign_keys=[created_by], back_populates='created_backups', lazy='selectin'
    )
    last_restore_user = db.relationship(
        'User', foreign_keys=[last_restore_by], back_populates='restored_backups', lazy='selectin'
    )
...
class BackupLog(TenantMixin, db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    backup_id = db.Column(
        db.Integer, db.ForeignKey('backups.id', ondelete='CASCADE'), nullable=False, index=True
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "log_type IN ('info', 'warning', 'error', 'success', 'debug')", name='chk_log_type'
        ),

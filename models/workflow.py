...
class WorkflowStep(TenantMixin, db.Model):
    ...
    )  # reception, doctor, lab, radiology, pharmacy, billing
    ...
    transfers_from = db.relationship(
        'WorkflowTransfer', foreign_keys='WorkflowTransfer.from_step_id', back_populates='from_step'
    )
    transfers_to = db.relationship(
        'WorkflowTransfer', foreign_keys='WorkflowTransfer.to_step_id', back_populates='to_step'
    )
...
class PatientWorkflow(TenantMixin, db.Model):
    ...
    current_step_id = db.Column(
        db.Integer,
        db.ForeignKey('workflow_steps.id', ondelete='RESTRICT'),
        nullable=False,
        index=True,
    )
    ...
    def get_priority_color(self):
        """لون الأولوية"""
        color_map = {'low': 'success', 'normal': 'primary', 'high': 'warning', 'urgent': 'danger'}
    ...
    def get_duration(self):
        ...
        return (datetime.now(UTC) - self.started_at).total_seconds() / 60
...
class WorkflowTransfer(TenantMixin, db.Model):
    ...
    workflow_id = db.Column(
        db.Integer,
        db.ForeignKey('patient_workflows.id', ondelete='CASCADE'),
        nullable=False,
        index=True,
    )
    from_step_id = db.Column(
        db.Integer,
        db.ForeignKey('workflow_steps.id', ondelete='SET NULL'),
        nullable=True,
        index=True,
    )
    to_step_id = db.Column(
        db.Integer,
        db.ForeignKey('workflow_steps.id', ondelete='SET NULL'),
        nullable=True,
        index=True,
    ...
    # تواريخ
    transferred_at = db.Column(db.DateTime, default=lambda: datetime.now(UTC))
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(UTC))

    # العلاقات
    workflow = db.relationship('PatientWorkflow', back_populates='transfers')
    from_step = db.relationship(
        'WorkflowStep', foreign_keys=[from_step_id], back_populates='transfers_from'
    )
    to_step = db.relationship(
        'WorkflowStep', foreign_keys=[to_step_id], back_populates='transfers_to'
    )
    ...
    def to_dict(self):
        ...
        return {
            ...
            'transferred_by_name': self.transferred_by_user.full_name
            if self.transferred_by_user
            else None,
...
class WorkflowQueue(TenantMixin, db.Model):
    ...
    # تواريخ
    queued_at = db.Column(db.DateTime, default=lambda: datetime.now(UTC))
    ...
    def get_wait_time(self):
        ...
        return (datetime.now(UTC) - self.queued_at).total_seconds() / 60

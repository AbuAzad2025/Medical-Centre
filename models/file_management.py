...
class FileUpload(TenantMixin, db.Model):
    ...
    )  # patient, visit, appointment, lab_result, radiology_result
    ...
    uploader = db.relationship(
        'User', foreign_keys=[uploaded_by], back_populates='uploaded_files', lazy='selectin'
    )
    permissions = db.relationship(
        'FilePermission', back_populates='file', lazy='dynamic', cascade='all, delete-orphan'
    )
    task_attachments = db.relationship(
        'TaskAttachment', back_populates='file', lazy='dynamic', cascade='all, delete-orphan'
    )
...
class FileCategory(TenantMixin, db.Model):
    ...
    creator = db.relationship(
        'User', foreign_keys=[created_by], back_populates='created_file_categories', lazy='selectin'
    )
...
class FilePermission(TenantMixin, db.Model):
    ...
    user = db.relationship(
        'User', foreign_keys=[user_id], back_populates='file_permissions', lazy='selectin'
    )
    granter = db.relationship(
        'User',
        foreign_keys=[granted_by],
        back_populates='granted_file_permissions',
        lazy='selectin',
    )

...
class Visit(TenantMixin, db.Model):
    ...
    )  # OPEN|CHECKED_IN|IN_PROGRESS|COMPLETED|CANCELLED
    ...
    visit_time = db.Column(db.DateTime, default=lambda: datetime.now(UTC))
    ...
    insurance_coverage_percentage = db.Column(
    ...
    force_payment_approved_by = db.Column(
    ...
    doctor = db.relationship(
        'User', back_populates='doctor_visits', foreign_keys=[doctor_id], lazy='selectin'
    )
    ...
    force_payment_approver = db.relationship(
        'User', foreign_keys=[force_payment_approved_by], lazy='selectin'
    )
    receipt_printer = db.relationship('User', foreign_keys=[receipt_printed_by], lazy='selectin')

    invoices = db.relationship(
        'Invoice', back_populates='visit', lazy='selectin', passive_deletes=True
    )
    lab_requests = db.relationship(
        'LabRequest', back_populates='visit', lazy='selectin', passive_deletes=True
    )
    radiology_requests = db.relationship(
        'RadiologyRequest', back_populates='visit', lazy='selectin', passive_deletes=True
    )
    ...
    patient_satisfaction_surveys = db.relationship(
        'PatientSatisfactionSurvey', back_populates='visit', lazy='selectin'
    )
    ...
    def payment_status_display(self):
        ...
        status_map = {'PENDING': 'معلق', 'PAID': 'مدفوع', 'PARTIAL': 'جزئي', 'DEBT': 'دين'}
    ...
    def to_dict(self) -> dict:
        return {
            ...
            'insurance_amount': str(
                Decimal(str(self.insurance_amount or 0)).quantize(Decimal('0.01'))
            )
            if self.insurance_amount is not None
            else None,
            'patient_share': str(Decimal(str(self.patient_share or 0)).quantize(Decimal('0.01')))

...
class DICOMStudy(TenantMixin, db.Model):
    ...
    # Status
    status = db.Column(
        db.String(50), default='RECEIVED'
    )  # RECEIVED, PENDING_REVIEW, REVIEWED, REPORTED, ARCHIVED
    ...
    series = db.relationship(
        'DICOMSeries', back_populates='study', lazy='dynamic', cascade='all, delete-orphan'
    )
...
class DICOMSeries(TenantMixin, db.Model):
    ...
    study_id = db.Column(
        db.Integer,
        db.ForeignKey('dicom_studies.id', ondelete='CASCADE'),
        nullable=False,
        index=True,
    )
    ...
    instances = db.relationship(
        'DICOMInstance', back_populates='series', lazy='dynamic', cascade='all, delete-orphan'
    )
...
class DICOMInstance(TenantMixin, db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    series_id = db.Column(
        db.Integer, db.ForeignKey('dicom_series.id', ondelete='CASCADE'), nullable=False, index=True
    )

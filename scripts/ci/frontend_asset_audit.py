...
def scan():
    ...
    tmpl_texts = {
        p.relative_to(TEMPLATES_DIR).as_posix(): p.read_text(encoding='utf-8', errors='ignore')
        for p in templates
    }
    dups = 0
    for rel, txt in tmpl_texts.items():
        ...
        if parent:
            ...
            if par in tmpl_texts:
                par_css = {
                    x for x in RE_URL_FOR_STATIC.findall(tmpl_texts[par]) if x.endswith('.css')
                }
    ...
    orphans = [
        f
        for f in orphans
        if f not in referenced and not any(f.startswith(x) for x in ['vendor/', 'img/'])
    ]
    ...
    for rel, txt in list(tmpl_texts.items())[:5]:
        ...
        if blocks:
            print(
                f'{rel} blocks: {blocks} extends={RE_EXTENDS.search(txt).group(1) if RE_EXTENDS.search(txt) else "-"} includes={len(RE_INCLUDE.findall(txt))}'
            )
    ...
    for s in samples:
        ...
        css = ','.join(
            [x.split('/')[-1] for x in RE_URL_FOR_STATIC.findall(t) if x.endswith('.css')][:2]
        )
        js = ','.join(
            [x.split('/')[-1] for x in RE_URL_FOR_STATIC.findall(t) if x.endswith('.js')][:2]
        )

...
def main() -> int:
    ...
    for item in items:
        ...
        print(
            f'  OK #{item.num} ({item.incident_id}): {item.description[:80]} — '
    ...
    for item in stale:
        ...
        if target:
            ...
            print('  └─ No target date set')
        print()

    print(f'Total: {len(items)} items — {len(resolved)} resolved, {len(stale)} stale.\n')
    print(f'Tip: Update statuses or target dates in {args.incident_log} to clear stale items.')
    return 1

...
def add_logger_import(content):
    ...
    if 'logger = logging.getLogger' not in content:
        ...
        for i, line in enumerate(lines):
            if not line.startswith('import ') and not line.startswith('from ') and line.strip() != '' and not line.startswith('#') and not line.startswith('"""') and not line.startswith("'''"):
                insert_at = i
                break
...
def main():
    ...
    for dirpath, dirnames, filenames in os.walk(ROOT):
        ...
        dirnames[:] = [d for d in dirnames if not d.startswith('.') and d not in ('__pycache__', 'node_modules', '.pytest_cache')]

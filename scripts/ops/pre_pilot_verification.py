...
def _phase1():
    ...
    _assert("PatientConsent has all required columns", required.issubset(cols), f"missing={required - cols}")
...
def _phase2():
    ...
    alert = SafetyAlert(
        check_type="allergy", severity=SafetyCheckSeverity.HARD_STOP,
        message="HARD STOP", override_requires="head_physician",
    )
    _assert("SafetyAlert HARD_STOP severity correct", alert.severity == SafetyCheckSeverity.HARD_STOP)
    ...
    _assert("Password policy rejects short passwords", not ok and any("12" in v for v in violations))

    ok, violations = svc.validate("longpassword123")
    _assert("Password policy rejects missing uppercase", not ok and any("uppercase" in v.lower() for v in violations))
    ...
    _assert("Password policy blocks personal info", not ok and any('username' in v.lower() for v in violations))
    ...
    with patch('services.password_policy_service.requests.get') as mock_get:
        ...
        mock_get.return_value = MagicMock(
            status_code=200, text=f"{sha1_suffix}:12345\nABCDE:1",
...
def _phase3():
    ...
    # Recovery test
    cb2 = CircuitBreaker("test_svc2", failure_threshold=1, recovery_timeout=0.1, success_threshold=1)
    try:
    ...
    from utils.api_security import limit_payload_size, require_content_type, verify_webhook_signature, sanitize_search_input
...
def _phase4():
    ...
    import inspect
    from routes.auth_routes import login, change_password, impersonate
    ...
    _assert("create_user calls set_password with user_context", "user_context" in inspect.getsource(create_user))
    _assert("reset_user_password uses generate_password", "generate_password" in inspect.getsource(reset_user_password))
    ...
    from routes.api_dashboard import dashboard_snapshot
    from routes.saas_routes import signup_organization, register_organization
    _assert("api_user has limit_payload_size", "limit_payload_size" in inspect.getsource(user_preferences))
    _assert("api_search has limit_payload_size", "limit_payload_size" in inspect.getsource(search_patients))
    _assert("saas signup has limit_payload_size", "limit_payload_size" in inspect.getsource(signup_organization))

    # Verify Stripe billing uses circuit breaker
    from services.stripe_billing_service import StripeBillingService
    _assert("stripe billing uses circuit_breaker_call", "circuit_breaker_call" in inspect.getsource(StripeBillingService))

    # Verify SMS uses circuit breaker
    from services.sms_service import SMSService
    _assert("sms service uses circuit_breaker_call", "circuit_breaker_call" in inspect.getsource(SMSService))

    # Verify webhook uses circuit breaker
    from services.webhook_service import _dispatch_single
    _assert("webhook dispatch uses circuit breaker", "breaker" in inspect.getsource(_dispatch_single))

    # Verify search sanitization in services
    from app.shared.search_service import SearchService
    _assert("search_patients uses sanitize_search_input", "sanitize_search_input" in inspect.getsource(SearchService.search_patients))
    from services.emergency_service import EmergencyService
    _assert("emergency list_cases uses sanitize_search_input", "sanitize_search_input" in inspect.getsource(EmergencyService.list_cases))
    from services.prescription_service import PrescriptionService
    _assert("search_medications uses sanitize_search_input", "sanitize_search_input" in inspect.getsource(PrescriptionService.search_medications))
...
def _phase5():
    ...
    _assert("Retention deadline calculated correctly", deadline.year in (2029, 2030), f"got {deadline.year}")

    old = datetime(2000, 1, 1, tzinfo=timezone.utc)
    _assert("Old record is eligible for action", svc.is_eligible_for_action(RetentionCategory.MEDICAL_RECORD, old))
    recent = datetime.now(timezone.utc)
    _assert("Recent record is NOT eligible", not svc.is_eligible_for_action(RetentionCategory.MEDICAL_RECORD, recent))
    ...
    c = PatientConsent(
        patient_id=1, consent_type='treatment', scope_description='Treatment consent',
        version=1, status='granted',
    )
    _assert("Consent version 1 is active", c.is_active())

    c2 = PatientConsent(
        patient_id=1, consent_type='marketing', scope_description='Marketing',
        version=1, status='granted',
    )
    c2.status = 'withdrawn'
    c2.withdrawn_at = datetime.now(timezone.utc)
    _assert("Withdrawn consent is inactive", not c2.is_active())

    c3 = PatientConsent(
        patient_id=1, consent_type='research', scope_description='Research',
        version=1, status='granted', expires_at=datetime(2020, 1, 1, tzinfo=timezone.utc),
    )
    ...
    log = ConsentAuditLog(
        consent_id=1, action='granted', patient_id=1,
...
def _phase6():
    ...
    from app_factory import _alert_admin
    ...
    _assert("Notification processor alerts admin", "_alert_admin" in source and "notif-processor" in source)
    _assert("Backup automation logs tracebacks", "traceback.format_exc()" in source)
    _assert("Backup automation alerts admin", "_alert_admin" in source and "backup-automation" in source)
    ...
    _assert("SaaS registration enforces password policy", "password_policy_service" in source3 or "weak_password" in source3)

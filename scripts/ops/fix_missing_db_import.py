...
def fix_file(filepath):
    ...
    has_db_import = bool(
        re.search(r'from app\.extensions import.*\bdb\b', content) or
        re.search(r'from app_factory import.*\bdb\b', content) or
        re.search(r'from extensions import.*\bdb\b', content)
    )
...
def main():
    ...
    for dirpath, dirnames, filenames in os.walk(ROOT):
        dirnames[:] = [d for d in dirnames if not d.startswith('.') and d not in ('__pycache__', 'node_modules', '.pytest_cache', 'scripts')]

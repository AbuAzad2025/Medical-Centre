...
def migrate_file(filepath):
    ...
    new = re.sub(r'(\w+)\.query\.filter_by\((.+?)\)\.all\(\)',
                 lambda m: f'db.session.execute(select({m.group(1)}).filter_by({m.group(2)})).scalars().all()',
                 content)
    if new != content:
    ...
    new = re.sub(r'(\w+)\.query\.filter_by\((.+?)\)\.first\(\)',
                 lambda m: f'db.session.execute(select({m.group(1)}).filter_by({m.group(2)})).scalars().first()',
                 content)
    if new != content:
    ...
    new = re.sub(r'(\w+)\.query\.filter_by\((.+?)\)\.one\(\)',
                 lambda m: f'db.session.execute(select({m.group(1)}).filter_by({m.group(2)})).scalars().one()',
                 content)
    if new != content:
    ...
    new = re.sub(r'(\w+)\.query\.filter_by\((.+?)\)\.one_or_none\(\)',
                 lambda m: f'db.session.execute(select({m.group(1)}).filter_by({m.group(2)})).scalars().one_or_none()',
                 content)
    if new != content:
    ...
    new = re.sub(r'db\.session\.query\((\w+)\)',
    ...
    if content != original:
        needs_select = bool(re.search(r'\bselect\(', content)) and 'from sqlalchemy import' not in content.split('select(')[0].split('\n')[-1]

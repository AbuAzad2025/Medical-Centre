...
def verify_webhook_signature(
    secret: str, header_name: str = 'X-Webhook-Signature', algorithm: str = 'sha256'
):
    """
    ...
    def decorator(f):
        ...
        def wrapper(*args, **kwargs):
            ...
            expected = hmac.new(
                secret.encode('utf-8'), body, getattr(hashlib, algorithm, hashlib.sha256)
            ).hexdigest()
            ...
            elif not hmac.compare_digest(expected, signature):
                return jsonify(
                    {
                        'success': False,
                        'error': 'Invalid webhook signature',
